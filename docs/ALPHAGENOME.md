...
ARCHCODE поддерживает опциональную интеграцию с AlphaGenome для сравнения контактных карт.

---

## Текущий статус

- **В v1.0**: используется **mock** (синтетические ответы) для разработки и тестов.
- **Для публикации**: валидация должна проводиться против **экспериментальных Hi-C** (например, Rao et al. 2014, ENCODE). Цель: Pearson r ≥ 0.7.
- Реальный AlphaGenome API планируется в v1.1 при наличии доступа.
