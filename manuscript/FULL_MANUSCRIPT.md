# ARCHCODE: An Analytical Framework for 3D Chromatin Structural Perturbation Scoring with Falsification-First Validation
...
**Background:** 3D chromatin structure influences gene regulation, and computational models based on loop extrusion have been proposed to predict how genetic variants alter chromatin topology and contribute to pathogenicity. However, systematic evaluation of whether such models provide discriminative signal beyond simple spatial and categorical features has been lacking. We developed ARCHCODE, a fast analytical mean-field loop extrusion simulator, and a falsification-first validation suite to test where structural models add genuine value and where they do not.

**Methods:** We processed 30,318 ClinVar variants across nine genomic loci (HBB, BRCA1, TP53, CFTR, MLH1, LDLR, SCN5A, TERT, GJB2) through ARCHCODE — an analytical mean-field loop extrusion simulator implementing Kramer kinetics for cohesin barrier crossing (parameters: α=0.92, γ=0.80, k_base=0.002; manually calibrated). For each variant, ARCHCODE computes a Structural Similarity Index (SSIM) comparing wild-type and mutant predicted contact maps. We introduce Local SSIM (LSSIM) — computed on a 50×50 submatrix centered on the variant — to normalize perturbation fraction across matrix sizes.

We constructed a six-module validation suite to stress-test structural claims:
(1) CTCF shuffle negative control — is signal specific to real CTCF architecture?
(2) Simple baseline comparison — does physics beat distance + severity features?
(3) Within-category discrimination — does the model separate pathogenic from benign within the same variant category?
(4) Ablation analysis — which components drive the signal?
(5) Cross-locus transfer — does a threshold trained on one locus generalize?
(6) Robustness — is signal stable across seeds and tissue contexts?

This suite ran 30 tests across all 9 loci, producing a comprehensive performance map of where structural signal survives stress-testing and where it fails.

**Results:** On HBB (n=1,103), ARCHCODE achieves AUC=0.977 for pathogenic vs benign classification. However, this signal is primarily category-driven: ablation shows position-only AUC=0.551, and within-category SSIM adds no discriminative value (p=1.0). The 27 "pearl" variants (VEP-blind, structurally disruptive) represent candidates for experimental prioritization but do not constitute independent pathogenicity discovery — they cluster in promoter/missense categories that receive stronger effect strengths by design.

The validation suite reveals systematic limitations:
- **CTCF shuffle:** On HBB (AUC>0.95), shuffled CTCF architectures produce identical discrimination — signal is geometry-driven, not biologically specific to real CTCF. On other loci (AUC 0.58–0.89), shuffle test passes trivially.
- **Simple baselines:** Random forest on distance-to-enhancer + category severity + position matches or beats ARCHCODE SSIM on 8/9 loci (RF AUC 0.68–0.95 vs SSIM 0.58–0.87). On GJB2, severity alone achieves AUC=0.957.
- **Within-category discrimination:** Median AUC across 25 category×locus tests = 0.52 (near-chance). TP53 is the sole exception: 4/4 categories show significant within-category signal (splice_region AUC=0.69, d=-0.78, surviving FDR correction across 25 tests).
- **Ablation:** Inverting the category→effect_strength mapping produces anti-predictive results (AUC=0.35 on TP53), confirming directional signal from category mapping, not from physics per se. Category-only AUC exceeds position-only by +0.14.
- **Cross-locus transfer:** An HBB-calibrated threshold (0.95) yields 0% sensitivity on all other loci — all variants score above threshold.
- **Hi-C correlation:** Pearson r=0.28–0.59 across 8 locus×cell-type combinations. Bayesian optimization confirms kinetics parameters are near-optimal (Δr=+0.0001), revealing that correlation is architecture-driven (distance decay, CTCF barriers), not kinetics-driven.

The validation suite itself — 30 tests, 9 loci, 6 modules — completes in under 30 seconds and is fully automated.
...
**Conclusions:** ARCHCODE is a fast analytical structural perturbation scoring framework, not a pathogenicity discovery engine. Our falsification-first validation reveals that: (1) broad structural pathogenicity claims do not survive stress-testing — category mapping, not 3D physics, drives discrimination; (2) simple spatial baselines match or beat the structural model on most loci; (3) cross-locus threshold transfer fails entirely; (4) within-category signal is limited to TP53 splice_region variants (AUC=0.69, surviving FDR). The validation suite — 30 automated tests across 9 loci — is our primary scientific contribution: a reusable, open benchmark for honest evaluation of 3D-genome variant models. We recommend that future structural variant interpretation frameworks be evaluated against this suite before claiming pathogenicity discovery.

**Limitations:**

- CTCF shuffle negative control: on HBB, shuffled architectures produce identical discrimination (AUC=0.979 vs 0.979 real) — signal is geometry-driven, not CTCF-specific
- Simple baselines (distance + severity) beat SSIM on 8/9 loci; on GJB2, severity alone = 0.957
- Within-category median AUC = 0.52 across 25 tests; only TP53 survives (4/4 categories, FDR-corrected)
- Cross-locus transfer: HBB threshold gives 0% sensitivity on all other loci
- Category→effect_strength mapping drives AUC; inverted mapping is anti-predictive (AUC=0.35)
- AUC=0.977 on HBB reflects category-distribution differences, not within-category prediction (position-only AUC=0.551)
- Analytical mean-field model; not full stochastic simulation; no cell-to-cell variation
- Hi-C correlation locus-dependent: MLH1 r=0.59; TP53 r=0.28–0.29
- Kinetics parameters confirmed near-optimal (Δr=+0.0001) — correlation is architecture-driven, not kinetics-driven
- Pearl variants require experimental validation; currently computational hypotheses only
- No patient-level validation; no clinical utility demonstrated

**Keywords:** chromatin loops, loop extrusion, cohesin, SSIM, structural perturbation scoring, VEP, regulatory variants, variant interpretation, mean-field simulation, falsification framework, validation suite, CFTR, TP53, BRCA1, MLH1, LDLR, SCN5A, TERT, GJB2,
...
## Cross-Locus Application and Transfer Analysis

We applied ARCHCODE to eight additional loci beyond HBB to test whether structural signal
transfers across genomic contexts. This is not a claim of generalization — rather, it is a
systematic mapping of where the model works and where it fails.

**Key finding:** Within-category discrimination is null on 7 of 8 additional loci
(median AUC = 0.52, near-chance). Cross-locus threshold transfer fails entirely: an
HBB-calibrated threshold (LSSIM < 0.95) yields 0% sensitivity on all 8 other loci.
The validation suite reveals that simple baselines (distance + severity) match or beat
SSIM on every locus tested.

---

## CFTR Locus: Null Within-Category Result
...
## TP53 Locus: Within-Category Signal Survives
...
## BRCA1 Locus: Largest Cohort, Null Within-Category
...
## MLH1 Locus: Highest Hi-C Correlation, Null Within-Category
...
## LDLR Locus: Tissue-Specific Validation (HepG2)
...
## SCN5A Locus: Cell-Type Mismatch Negative Control
...
## What Structural Models Can and Cannot Reveal — Lessons from ARCHCODE

We began this study with a clear question: can an analytical mean-field loop extrusion simulator identify pathogenic signal in genetic variants that sequence-based predictors miss? After processing 30,318 ClinVar variants across nine loci and running 30 systematic falsification tests, the answer is more nuanced than our initial hypothesis.

ARCHCODE correctly stratifies variants by functional category: mean SSIM follows the biologically expected ranking from nonsense (0.8753) through missense (0.9526) to synonymous (0.9989). On HBB, this produces AUC = 0.977 for pathogenic vs benign classification. However, our ablation analysis reveals that this AUC is entirely attributable to the category→effect_strength mapping: a position-only control (fixed effectStrength = 0.3) yields AUC = 0.551, indistinguishable from chance. The model is a structural property of the occupancy-scaling approach: perturbation magnitude is assigned by functional category, not by genomic position within category.

The validation suite we constructed reveals systematic limitations that are the primary scientific contribution of this work:

**CTCF shuffle negative control.** On HBB (where AUC > 0.95), shuffled CTCF architectures produce identical discrimination (AUC = 0.979 for both real and shuffled). This means the signal is geometry-driven — any arrangement of barriers yields the same category-driven separation. On other loci (AUC 0.58–0.89), the shuffle test passes trivially because the overall AUC is already below the 0.95 threshold.

**Simple baselines.** Random forest on three features — distance to nearest enhancer, category severity score, and normalized position — matches or beats ARCHCODE SSIM on 8 of 9 loci (RF AUC 0.68–0.95 vs SSIM 0.58–0.87). On GJB2, category severity alone achieves AUC = 0.957, exceeding SSIM's 0.871. This is the most damaging result: the physics simulation adds no genuine discriminative value beyond knowing what category a variant is and where it sits relative to enhancers.

**Within-category discrimination.** The critical test for any "structural pathogenicity" claim: can the model separate pathogenic from benign variants _within the same consequence category_? Across 25 category×locus tests with sufficient sample size (≥5 pathogenic and ≥5 benign), median AUC = 0.52 — essentially chance. TP53 is the sole exception: all 4 testable categories show significant within-category signal (splice_region AUC = 0.69, d = -0.78; other AUC = 0.75; synonymous AUC = 0.58; intronic AUC = 0.56), surviving FDR correction across 25 tests. This is a genuine signal island, but it is one locus, one set of categories, with modest effect sizes.

**Cross-locus transfer.** An HBB-calibrated threshold (LSSIM < 0.95) yields 0% sensitivity on all 8 other loci — every variant scores above the threshold. This is expected given the different matrix sizes and locus architectures, but it means there is no universal "structural pathogenicity" threshold.

**Ablation analysis.** Inverting the category→effect_strength mapping (making benign categories get strong effects and pathogenic categories get weak effects) produces anti-predictive results (AUC = 0.35 on TP53). This confirms that the directional signal comes from the category mapping itself, not from any physics-based positional sensitivity.

What _does_ work: the Hi-C correlation trajectory (GM12878 r = 0.16 → K562 r = 0.53 → K562 95 kb r = 0.59) demonstrates that the model captures genuine chromatin contact topology when supplied with cell-type-appropriate anchors. Bayesian optimization confirms kinetics parameters are near-optimal (Δr = +0.0001), revealing that correlation is architecture-driven (distance decay, CTCF barriers, MED1 landscape), not kinetics-driven.

We have not confirmed the "Loop That Stayed" mechanism. We have generated computational predictions that specific promoter-region HBB variants show structural disruption in our model. Whether this reflects genuine regulatory biology requires experimental testing.

## What ARCHCODE Actually Measures — Category Mapping, Not Structural Discovery

The 130 discordant variants (36.8% of HBB dataset) illuminate the complementary nature of structural and sequence-based approaches, but also reveal the self-inflicted nature of ARCHCODE's apparent success. The 128 VEP-only variants — those called pathogenic by VEP but not by ARCHCODE — consist predominantly of missense variants acting through protein structural perturbation, a mechanism ARCHCODE cannot detect. This is expected and appropriate: ARCHCODE does not model protein folding.

The reverse discordance — 20 pearl candidates detected by ARCHCODE but assigned low pathogenicity by VEP — is more nuanced than our initial framing suggested. For the promoter-region pearls, the mechanism is clear in model terms: VEP annotates upstream_gene_variant as low-impact because the standard consequence weighting does not account for LCR–HBB contact disruption. ARCHCODE's physics-based model, by contrast, is sensitive to changes at positions it has annotated as enhancer elements, regardless of whether these positions match a recognized splice or coding motif.

However, our validation suite reveals that this "structural blind spot" is partially self-created. The category→effect_strength mapping assigns strong perturbations to nonsense, frameshift, splice, and promoter variants (effectStrength 0.1–0.3) and weak perturbations to intronic, synonymous variants (effectStrength 0.8–0.9). This mapping is biologically reasonable — severe variant categories _should_ cause larger effects — but it means that the model's discriminative power comes from encoding prior knowledge about category severity, not from discovering novel structural mechanisms.

The "pearl" variants are not invisible to sequence tools because of some hidden 3D mechanism. They are VEP-blind because they fall in promoter/missense/splice categories that VEP scores conservatively for regulatory variants, while ARCHCODE assigns them strong effect strengths by design. The 15 promoter pearls cluster in a 73 bp window (chr11:5,227,099–5,227,172) at a median distance of 147 bp from the HBB promoter enhancer. A simple distance-to-enhancer threshold among VEP-invisible variants achieves AUC = 0.877. The pearls differ from VEP-invisible benign variants (intronic, synonymous) primarily in category — there is zero category overlap — not in any discovered 3D physics.

This complementarity mirrors the relationship between different assessment tools. VEP is sensitive to sequence-level defects in protein-coding and canonical splice sequences. ARCHCODE is sensitive to regulatory topology — changes at simulated CTCF anchors and enhancer elements within the simulation window. A variant outside both detection ranges will be missed by both; a variant detected by only one provides hypothesis-generating signal for follow-up, not confirmed pathogenicity.

**Integrative CADD benchmark.** We obtained CADD v1.7 phred scores for 20,029 of 30,318 ClinVar variants (66.1% coverage). The 53 ARCHCODE-only variants (LSSIM < 0.95, CADD < 20) reveal locus-dependent performance: all 25 HBB variants are ClinVar Pathogenic (15 promoter, 8 missense, 2 splice_region), while 25 of 26 BRCA1 ARCHCODE-only variants are ClinVar Benign/Likely benign, reflecting threshold artifacts at the boundary of the benign LSSIM distribution in this 400 kb locus. This asymmetry establishes ARCHCODE's domain of applicability: structural discrimination is strongest for regulatory variants in compact loci with enhancer–promoter architecture (HBB: 0/750 benign false positives at threshold 0.95, 79.6% sensitivity), and weakest for large loci where pathogenicity is protein-mediated (BRCA1: 26/3,620 benign false positives, 0.75% sensitivity).

**Model independence and its limits.** ARCHCODE is mechanistically orthogonal to VEP — it receives no input from VEP scores, SIFT predictions, or ClinVar classifications. The two models operate on fundamentally different biological layers. However, the absence of missense sensitivity in ARCHCODE is a topological feature of the model: missense variants that cause disease through protein misfolding operate entirely at the protein level, a dimension invisible to chromatin topology simulation. For missense variants, sequence-based VEP tools remain the appropriate primary tool.
...
**1. Category-driven AUC, not within-category prediction.** The primary limitation revealed by our validation suite: ARCHCODE's AUC of 0.977 on HBB is attributable to the category→effect_strength mapping, not to positional sensitivity within categories. Position-only control AUC = 0.551. Within-category SSIM adds no discriminative value on HBB (p = 1.0). This means the model characterizes the variant catalog, not structural prediction power.

**2. Simple baselines match or beat SSIM on 8/9 loci.** Random forest on three features (distance to enhancer, category severity, position) achieves higher AUC than ARCHCODE SSIM on BRCA1 (0.826 vs 0.691), TP53 (0.845 vs 0.669), CFTR (0.731 vs 0.665), MLH1 (0.817 vs 0.706), LDLR (0.795 vs 0.647), SCN5A (0.678 vs 0.584), TERT (0.893 vs 0.844), and GJB2 (0.948 vs 0.871). On GJB2, category severity alone achieves AUC = 0.957. The physics simulation adds no genuine discriminative value.

**3. Within-category discrimination fails on 7/8 loci.** Median AUC across 25 category×locus tests = 0.52 (near-chance). Only TP53 shows significant within-category signal (4/4 categories, surviving FDR), with modest effect sizes (splice_region AUC = 0.69, d = -0.78). This is a genuine signal island but not sufficient to support broad structural pathogenicity claims.

**4. Cross-locus transfer fails entirely.** An HBB-calibrated threshold yields 0% sensitivity on all 8 other loci. There is no universal "structural pathogenicity" threshold — each locus requires independent calibration.

**5. CTCF shuffle: signal is geometry-driven, not biologically specific.** On HBB, shuffled CTCF architectures produce identical discrimination (AUC = 0.979). The LSSIM metric responds to any arrangement of barriers, not specifically to the real CTCF architecture.

**6. Ablation confirms category mapping drives signal.** Inverted effect strengths produce anti-predictive results (AUC = 0.35 on TP53). The directional signal comes from the category mapping, not from physics-based positional sensitivity.

**7. Hi-C correlation locus-dependent.** Pearson r = 0.28–0.59 across 8 locus×cell-type combinations. Best at MLH1 r = 0.59 and HBB 95 kb r = 0.59; worst at TP53 r = 0.28–0.29. Explains 8–35% of variance in experimental contact frequencies.

**8. Analytical mean-field model.** The model lacks polymer physics baseline, A/B compartmentalization, stochastic cell-to-cell variation, and multi-cohesin dynamics. It is a fast approximation, not a full biophysical simulation.

**9. Kinetics parameters confirmed near-optimal; no room for improvement.** Bayesian optimization yielded Δr = +0.0001. Hi-C correlation is architecture-driven (distance decay, CTCF barriers, MED1 landscape), not kinetics-driven.

**10. Pearl variants require experimental validation.** The 27 HBB pearl candidates are computational hypotheses only. No RT-PCR, Capture-C, or functional assay has been performed. No clinical reclassification should be considered without experimental confirmation.

**11. No patient-level validation.** ARCHCODE predictions have not been correlated with patient phenotypes, disease severity, or clinical outcomes.

## Implications for 3D-Genome Model Evaluation

The validation suite we present here — 30 automated tests across 9 loci — is the primary scientific contribution of this work. It establishes a falsification-first benchmark that any 3D-genome variant interpretation model should pass before claiming pathogenicity discovery:

1. **CTCF shuffle:** Does signal survive random barrier rearrangement? If not, the metric responds to geometry, not biology.
2. **Simple baselines:** Does the model beat distance-to-enhancer + category severity? If not, the physics adds no value.
3. **Within-category:** Does the model separate pathogenic from benign within the same consequence category? If not, the signal is category-driven.
4. **Cross-locus transfer:** Does a threshold trained on one locus generalize? If not, there is no universal "structural pathogenicity" score.
5. **Ablation:** Does inverting the category→effect mapping destroy prediction? If so, the mapping drives the signal.
6. **Robustness:** Is signal stable across seeds and tissue contexts?

On ARCHCODE, 12 of 30 tests FAIL, 9 are WARNING, and only 9 PASS. The FAIL results are concentrated in the tests that matter most for the "structural pathogenicity discovery" claim: category-driven AUC, baseline superiority, within-category null, and transfer failure.

What remains honest and useful: ARCHCODE is a sub-second structural perturbation scorer that produces interpretable contact maps for visualization and hypothesis generation. The 27 HBB pearl variants are candidates for experimental prioritization. The TP53 splice_region signal (AUC=0.69, FDR-corrected) is a genuine within-category effect worth further study. The validation suite itself is reusable and should be applied to any future 3D-genome model before it claims to discover "invisible" pathogenicity.
