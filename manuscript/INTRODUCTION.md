...
2. **Calculate SSIM scores** quantifying structural disruption and classify variants using physics-informed thresholds (SSIM <0.70 = structurally disrupted).
...
We hypothesize the existence of a novel variant class—tentatively termed **"The Loop That Stayed"**—characterized by preserved chromatin loop architecture (SSIM 0.50–0.60) concurrent with disruption of splice regulatory elements. If confirmed, such variants would be systematically underestimated by sequence-based predictors, which internalize a structure-function coupling assumption that inverts in this mechanistic context. Our comparative analysis of ARCHCODE structural outputs and Ensembl VEP scores across 353 _HBB_ variants is designed to test whether this class exists, how frequently it arises, and whether discordant predictions carry actionable clinical signal for VUS reclassification.
