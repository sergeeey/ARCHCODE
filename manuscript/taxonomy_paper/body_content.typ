...
*Signal amplitude reveals a two-regime rather than continuous structure.* Across 14 configured loci, the effect-size distribution (Δ_pb, mean pathogenic minus mean benign LSSIM) is bimodal: HBB reaches Δ_pb = 0.111, while all remaining loci fall in the range 0.000--0.019 — a 5.9-fold gap to the nearest locus (TERT). A 14-locus Pearson correlation of discrete tissue_match score with Δ_pb yields r = 0.53 (p = 0.049); however, the Spearman rank correlation ρ = 0.48 (p = 0.082) is borderline, and removal of HBB reduces the association to r = 0.37 (p = 0.22), confirming that HBB is the dominant leverage point. These analyses indicate that ARCHCODE currently operates in two qualitatively distinct regimes: Regime 1 — tissue-specific genes in compact TADs with focused simulation windows (≤ 95 kb; currently represented only by HBB) — where structural signal is strong and robustly tissue-matched; and Regime 2 — housekeeping or widely expressed genes in 300--400 kb windows — where structural signal is detectable but 5--20-fold weaker. The tissue-specificity gradient described above reflects this regime structure rather than a smooth relationship between tissue context and signal amplitude. Expanding the Regime 1 panel by identifying loci with comparable TAD compactness and tissue-specificity (candidates: KLF1, HBA2, GATA2 in erythroid lineage) is a prerequisite for robust cross-locus quantitative modeling.
