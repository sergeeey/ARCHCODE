...
#strong[Methods:] ARCHCODE implements Kramer kinetics for cohesin barrier crossing (α=0.92, γ=0.80; calibrated to literature ranges). For each variant, the simulator computes a Structural Similarity Index (SSIM) comparing wild-type and mutant contact maps. We analyzed 25,272 ClinVar variants across 6 loci (HBB, CFTR, TP53, BRCA1, MLH1, LDLR) plus SCN5A as negative control (27,760 total). Local SSIM (LSSIM) --- computed on a 50×50 submatrix centered on the variant --- resolves matrix-size dilution across loci.

#strong[Results:] On HBB (n=1,103), ARCHCODE stratified variants by functional category: loss-of-function classes showed severe structural disruption (mean SSIM 0.88--0.90), while neutral classes preserved structure (mean SSIM \> 0.99). Twenty "pearl" variants were identified --- invisible to VEP (score \< 0.30) but structurally disruptive (SSIM \< 0.95). ROC analysis yielded AUC = 0.976; however, a position-only control (fixed effectStrength = 0.3) produced AUC = 0.551, confirming the high AUC reflects category-distribution differences. Within-category testing across all loci confirmed primarily null results (ΔAUC \< 0.02). Hi-C validation against K562, MCF7, and HepG2 data yielded correlations r = 0.29--0.59 (all p \< 10#super[-82]).
