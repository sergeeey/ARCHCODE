...
export function Simulator() {
    ...
    if (loops.length > prev) {
      ...
      setCurrentRun((prevRun) => {
        ...
        const newEvents = newlyFormed.map((loop) => ({
          ...
          payload: {
            left: loop.leftAnchor,
            right: loop.rightAnchor,
            sizeBp: loop.rightAnchor - loop.leftAnchor,
          },
        }));
  ...
  const activeCohesins = useMemo(
    () => engine.getCohesins().filter((c) => c.active).length,
    [engine, stepCount],
  );
  ...
  const handleSitesLoaded = useCallback(
    (sites: CTCFSite[], _cellLine?: string) => {
      setCTCFSites(sites);
      if (sites.length > 0) {
    ...
    setCurrentRun((prev) => {
      ...
      return {
        ...
        results: {
          ...prev.results,
          steps: finalStep,
          loopsFormed: prev.events.filter((e) => e.type === "loop_formed")
            .length,
          uniqueLoopsFormed: (() => {
            const keys = new Set(
              prev.events
                ...
                .map(
                  (e) =>
                    `${(e.payload as { left: number; right: number }).left}-${(e.payload as { left: number; right: number }).right}`,
                ),
            );
            return keys.size;
          })(),
  ...
  const handleApplyParamsFromLoaded = useCallback(() => {
    ...
    const chromosome =
      run.model.chromosome ?? run.model.ctcfSites[0]?.chrom ?? "chr1";
    setCTCFSites(
      run.model.ctcfSites.map((s) =>
        createCTCFSite(s.chrom ?? chromosome, s.pos, s.orient, 1.0),
      ),
    );
    setStepCount(0);
  }, [loadedRun]);
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0">
        {viewMode === "tube" && (
          <GenomeViewer
            engine={engine}
            stepCount={stepCount}
            pulseLoops={pulseLoops}
          />
        )}
        {viewMode === "linear" && (
          <SimulationViewer engine={engine} pulseLoops={pulseLoops} />
        )}
        {viewMode === "helix" && (
          <DNAHelixViewer
      ...
      >
        <h3 className="m-0 mb-2.5 text-sm font-semibold text-[var(--text-heading)]">
          ARCHCODE v0.3.1
        </h3>
        <div className="text-[var(--text-label)]">
          CTCF Sites:{" "}
          <span className="tabular-nums text-[var(--text-heading)]">
            {ctcfSites.length}
          </span>
        </div>
        <Tooltip
          ...
          content={
            <>
              Количество событий временной стабилизации хроматиновых петель за
              время симуляции.
              <br />
              <br />
        ...
        >
          <div className="text-[var(--text-label)]">
            Loops:{" "}
            <span className="tabular-nums text-[var(--accent-live)]">
              {loops.length}
            </span>
          </div>
        </Tooltip>
        ...
        <div className="mt-2.5 text-[11px] text-[var(--text-label)]">
        ...
        <div className="mt-2 text-[11px] text-[var(--text-label)] border-t border-[rgba(255,255,255,0.1)] pt-2">
          {isRunning && activeCohesins > 0 && (
            <span className="text-[var(--accent-live)]">
              🟢 Активная экструзия
            </span>
          )}
          {isRunning && activeCohesins === 0 && (
          ...
          {!isRunning && (
            ...
            >
              <span className="text-[var(--accent-muted)] cursor-help">
                🔵 Плато достигнуто — новых петель нет
              </span>
            </Tooltip>
          )}
        </div>
      ...
      <div className="panel-glass absolute top-5 right-5 flex gap-0.5 rounded-lg overflow-hidden font-mono text-xs z-10">
        {(["tube", "linear", "helix"] as const).map((mode) => (
          <button
            ...
            style={{
              background:
                viewMode === mode ? "var(--accent-live)" : "transparent",
      ...
      <AnimatePresence>
        {viewMode === "helix" && (
          ...
          >
            <div className="mb-3 font-semibold text-[var(--text-heading)]">
              DNA Helix
            </div>
            <div className="space-y-3">
              <Slider
                ...
                min={8}
                max={48}
                value={helixBasePairs}
                onValueChange={setHelixBasePairs}
                valueLabel={String(helixBasePairs)}
              />
              <Slider
                label="Twist"
                min={0.2}
                max={1.2}
                step={0.01}
                value={helixTwist / ((2 * Math.PI) / 10.5)}
                onValueChange={(v) => setHelixTwist((v * (2 * Math.PI)) / 10.5)}
                valueLabel={(helixTwist * (10.5 / (2 * Math.PI))).toFixed(2)}
              />
              <Slider
                label="Scale"
                min={0.5}
                ...
                step={0.1}
                value={helixScale}
                onValueChange={setHelixScale}
                valueLabel={helixScale.toFixed(2)}
              />
            </div>
          </motion.div>
        )}
              ...
              <div className="flex-1 overflow-auto p-5 flex flex-col gap-5">
                ...
                {sidebarTab === "data" && (
                  <>
                    ...
                    <ENCODEDownloader
                      onSitesLoaded={(sites, cellLine) =>
                        handleSitesLoaded(sites, cellLine)
                      }
                    />
                    <LoopDashboard
                      loops={loops}
                ...
                {sidebarTab === "legend" && (
                  <Panel title="Legend">
                    <div className="grid gap-1.5 text-xs text-[var(--text-label)]">
                      <div>
                        <span className="text-[var(--accent-live)]">F</span>{" "}
                        CTCF Forward
                      </div>
                      <div>
                        <span className="text-[var(--accent-danger)]">R</span>{" "}
                        CTCF Reverse
                      </div>
                      <div>
                        <span className="text-[var(--accent-live)]">Loop</span>{" "}
                        Formed
                      </div>
                      <div>
