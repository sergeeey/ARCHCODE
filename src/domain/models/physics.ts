    ...
    for (const element of elements.values()) {
        if (element.type === 'HETEROCHROMATIN') {
            ...
            if (distance < HETEROCHROMATIN_PARAMS.DETECTION_RANGE_BP) {
                return COHESIN_PARAMS.EXTRUSION_SPEED_BP_PER_S * HETEROCHROMATIN_PARAMS.FRICTION_FACTOR;
            }
        }
    }
...
export function stepPhysics(
    ...
    for (const loop of loops) {
        ...
        const leftSpeed = calculateExtrusionSpeed(newLeft, elements) * timeStepSeconds;
        const rightSpeed = calculateExtrusionSpeed(newRight, elements) * timeStepSeconds;
        ...
        if (!stalledByLeft && newLeft > 0) {
            ...
            for (const element of elements.values()) {
                if (element.type === 'CTCF_FORWARD' || element.type === 'CTCF_REVERSE') {
        ...
        if (!stalledByRight && newRight < genomeLength) {
            ...
            for (const element of elements.values()) {
                if (element.type === 'CTCF_FORWARD' || element.type === 'CTCF_REVERSE') {
                    const ctcf = element as CTCFSite;
                    // Check if we would cross this CTCF
                    if (ctcf.position > newRight && ctcf.position <= newRight + rightSpeed) {
