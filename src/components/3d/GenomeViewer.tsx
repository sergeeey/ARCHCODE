...
function isPulsing(loop: Loop, pulseLoops: Loop[]): boolean {
  return pulseLoops.some(
    (p) =>
      p.leftAnchor === loop.leftAnchor && p.rightAnchor === loop.rightAnchor,
  );
}

/** Дуга петли с кратким cyan-glow при событии loop_formed (0.5–1 с) */
function LoopArcMesh({
  curve,
  pulsing,
}: {
  ...
  useFrame(() => {
    ...
    mat.emissiveIntensity =
      EMISSIVE_NORMAL + (EMISSIVE_PEAK - EMISSIVE_NORMAL) * (1 - t);
  });
  ...
  const loopCurves = useMemo(() => {
    return loops.map((loop) => {
      ...
      const p2 = new THREE.Vector3(
        Math.cos(midAngle) * (radius + height),
        Math.sin(midAngle) * (radius + height),
        5,
      );
    ...
    <group>
      ...
      {loopCurves.map((curve, i) => (
        <LoopArcMesh
          key={i}
          curve={curve}
          pulsing={isPulsing(loops[i], pulseLoops)}
        />
      ))}
    </group>
  );
...
export const GenomeViewer: React.FC<GenomeViewerProps> = ({
  engine,
  stepCount = 0,
  pulseLoops = [],
}) => {
  const data = useMemo(() => {
    if (engine) {
      return {
        genomeLength: engine.genomeLength,
        loops: engine.getLoops(),
        ctcfSites: engine.ctcfSites,
      };
    }
    return {
      genomeLength: 100000,
      loops: [] as Loop[],
      ctcfSites: [] as CTCFSite[],
    };
  }, [engine, stepCount]);
