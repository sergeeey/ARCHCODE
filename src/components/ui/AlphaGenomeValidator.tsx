...
export const AlphaGenomeValidator = ({
  engine,
  apiKey,
}: AlphaGenomeValidatorProps) => {
  const [validating, setValidating] = useState(false);
  const [result, setResult] = useState<ValidationResult | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [activeView, setActiveView] = useState<
    "alphagenome" | "archcode" | "diff"
  >("diff");
  ...
  const getCurrentMatrix = () => {
    ...
    switch (activeView) {
      case "alphagenome":
        return result.alphaGenomeMap;
      case "archcode":
        return result.archcodeMap;
      case "diff":
        return result.diffMap;
      default:
        return null;
    }
  };
        ...
        <>
          ...
          >
            {result.pearsonCorrelation > 0.8 ? (
              <>
                ✅ <strong>Excellent agreement!</strong> Your simulation closely
                matches AlphaGenome predictions.
              </>
            ) : result.pearsonCorrelation > 0.6 ? (
              <>
                ⚠️ <strong>Good agreement.</strong> Some differences observed.
                Check parameters.
              </>
            ) : (
              <>
