...
export function Tooltip({ title, content, children, delayMs = DELAY_MS }: TooltipProps) {
        ...
        >
            ...
            {visible && (
                <div
                    ...
                    role="tooltip"
                >
                    <div className="font-semibold text-[var(--text-heading)] mb-1">{title}</div>
                    <div className="text-[var(--text-label)] leading-relaxed whitespace-normal">{content}</div>
                </div>
            )}
        </span>
    );
