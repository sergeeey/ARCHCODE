...
export const ContactMatrixViewer = ({
  matrix,
  title = "Contact Matrix",
}: ContactMatrixViewerProps) => {
...
export const PSCurveViewer = ({ psCurve, fit }: PSCurveViewerProps) => {
  ...
  if (psCurve.distances.length === 0) {
    return (
      <Panel title="P(s) Curve" className="font-mono">
        <div className="text-[var(--text-label)] text-sm">
          No data available
        </div>
      </Panel>
    );
  ...
  const logContacts = psCurve.contacts.map((c) =>
    Math.log10(Math.max(c, 1e-10)),
  );
  ...
  const pathData = logDistances
    .map(
      (x, i) => `${i === 0 ? "M" : "L"} ${scaleX(x)} ${scaleY(logContacts[i])}`,
    )
    .join(" ");
  ...
  return (
    <Panel title="P(s) Curve (log-log)" className="font-mono">
      <svg width={canvasWidth} height={canvasHeight} className="block">
        ...
        <text
          x={canvasWidth - padding.right - 30}
          y={canvasHeight - 10}
          fill="var(--text-label)"
          fontSize="10"
          fontFamily="monospace"
      ...
      <div className="mt-4 p-3 rounded-[var(--radius-sm)] bg-[rgba(0,240,255,0.06)] border border-[rgba(255,255,255,0.08)] text-xs">
        ...
        <div className="mt-2 flex gap-4 tabular-nums">
          <span className="text-[var(--text-label)]">
            α:{" "}
            <span className="text-[var(--accent-live)] font-medium">
              {fit.alpha.toFixed(3)}
            </span>
          </span>
          <span className="text-[var(--text-label)]">
        ...
        <div className="mt-1 text-[10px] text-[var(--text-label)]">
          Expected α ≈ -1.0
        </div>
      </div>
    </Panel>
