...
export function ExperimentPanel({
    ...
    reader.onload = () => {
      try {
        ...
        if (!validateImportedRun(data)) {
          alert(
            "Неверный формат файла. Ожидается ARCHCODE Run (schemaVersion 1.0).",
          );
          return;
        }
        onImport(data);
  ...
  const handleCopySummary = async () => {
    ...
    if (!run) {
      alert(
        "Нет данных для копирования. Запустите эксперимент или загрузите run.",
      );
      return;
    }
        ...
        {loadedRun && (
          <div className="rounded-lg border border-[rgba(255,255,255,0.1)] bg-black/20 p-3 space-y-2">
            ...
            <div className="text-[var(--text-label)] text-[10px]">
              {loadedRun.run.createdAt.slice(0, 10)} · {loadedRun.run.mode} ·{" "}
              {loadedRun.results.loopsFormed} loops
            </div>
...
function RunSummary({ run }: { run: ArchcodeRun }) {
  ...
  return (
    <div className="rounded-lg border border-[rgba(255,255,255,0.1)] bg-black/20 p-3 space-y-2 text-[var(--text-label)]">
      <div className="font-semibold text-[var(--text-heading)] text-[11px]">
        Run Summary
      </div>
      <div className="grid gap-1 text-[10px]">
        <div>
          ID: {r.id.slice(0, 12)} · {r.createdAt.slice(0, 16)}
        </div>
        <div>
        ...
        <div>
          Steps: {res.steps} · Loops (events): {res.loopsFormed}
          {res.uniqueLoopsFormed != null
            ? ` · Unique pairs: ${res.uniqueLoopsFormed}`
            : ""}{" "}
          · Convergent: {res.convergentPairs}
        </div>
      </div>
        ...
        <div className="text-[10px]">
          ...
          <ul className="list-disc pl-4 mt-0.5">
            {res.stablePairs.slice(0, 3).map((p, i) => (
              <li key={i}>
                {p.left.toLocaleString()} – {p.right.toLocaleString()} bp
                {p.sizeBp != null
                  ? ` (${(p.sizeBp / 1000).toFixed(1)} kb)`
                  : ""}
              </li>
            ))}
          </ul>
