...
export const LoopDashboard = ({ loops, isRunning, stepCount }: LoopDashboardProps) => {
  ...
  const statusColor = !isRunning ? 'var(--text-label)' : loops.length > 0 ? 'var(--accent-live)' : 'var(--accent-muted)';
  const statusText = !isRunning ? 'Idle' : loops.length > 0 ? 'Loops Forming' : 'Running (No Loops)';
    ...
    <Panel className="font-mono">
      <div className="flex items-center gap-3 mb-4">
        <div
          ...
          style={{ background: statusColor, boxShadow: isRunning && loops.length > 0 ? `0 0 8px ${statusColor}` : undefined }}
        />
        <div>
          <div className="text-[11px] text-[var(--text-label)]">Status</div>
          <div className="text-sm font-semibold" style={{ color: statusColor }}>{statusText}</div>
        </div>
      </div>

      <div className="bg-[rgba(255,255,255,0.04)] rounded-[var(--radius-sm)] p-4 mb-4 text-center border border-[rgba(255,255,255,0.06)]">
        <div className="text-[11px] text-[var(--text-label)] mb-1">Loops Formed</div>
        <div className="text-3xl font-bold tabular-nums relative" style={{ color: displayLoops > 0 ? 'var(--accent-live)' : 'var(--text-heading)' }}>
          {displayLoops}
          {lastAdded !== null && (
            <span className="absolute right-0 top-0 text-sm text-[var(--accent-live)]">+{lastAdded}</span>
          )}
        </div>
      </div>

      <div className="flex justify-between text-[11px] mb-2 text-[var(--text-label)]">
        <span>Rate: <span className="tabular-nums font-medium" style={{ color: loopRate > 0 ? 'var(--accent-live)' : 'inherit' }}>{(loopRate ?? 0).toFixed(2)}</span> /100 steps</span>
        <span>Step: <span className="tabular-nums">{(stepCount ?? 0).toLocaleString()}</span></span>
      </div>

      <div className="mb-3">
        <div className="text-[10px] text-[var(--text-label)] mb-1">Loop History</div>
        <canvas
          ref={canvasRef}
          width={300}
          height={56}
      ...
      <div className="mb-1">
        <div className="flex justify-between text-[10px] text-[var(--text-label)] mb-1">
          ...
          <span className="tabular-nums">{Math.min((displayLoops / 3) * 100, 100).toFixed(0)}%</span>
        </div>
        <div className="h-1 rounded-full bg-[rgba(255,255,255,0.1)] overflow-hidden">
          <div
            ...
            style={{
              ...
              background: displayLoops >= 3 ? 'var(--accent-live)' : 'var(--accent-dna)',
            }}
          />
        </div>
      </div>
