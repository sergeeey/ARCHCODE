...
export class FountainLoader implements ISpatialLoader {
    ...
    constructor(options: {
        ...
        const sorted = [...this.signalBins].filter(x => isFinite(x) && x >= 0).sort((a, b) => a - b);
        this.medianSignal = sorted.length > 0
        ...
        this.binProbs = sum > 0 ? rawProbs.map(p => p / sum) : rawProbs.map(() => 1 / n);
