  ...
  devtools(
    (set, get) => ({
      ...
      addLoop: (atPosition) =>
        set((state) => {
          const position =
            atPosition ?? Math.floor(random() * state.config.genomeLength);
      ...
      stepSimulation: () =>
        set((state) => {
          ...
          return {
            ...
            biologicalTime:
              state.biologicalTime + state.config.biologicalTimeScale,
          };
        }),
...
export const selectBiologicalTime = (state: GenomeStore) =>
  state.biologicalTime;
