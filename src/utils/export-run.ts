...
export function validateImportedRun(data: unknown): data is ArchcodeRun {
    ...
    if (!o.app || typeof (o.app as Record<string, unknown>).name !== 'string') return false;
    if (!o.run || typeof o.run !== 'object') return false;
    const run = o.run as Record<string, unknown>;
    if (typeof run.id !== 'string' || typeof run.createdAt !== 'string') return false;
    if (typeof run.mode !== 'string' || !['tube', 'linear', 'helix'].includes(run.mode)) return false;
    if (typeof run.status !== 'string' || !['running', 'plateau', 'stopped', 'error'].includes(run.status)) return false;
    if (!o.params || typeof o.params !== 'object') return false;
    const params = o.params as Record<string, unknown>;
    if (typeof params.extrusionSpeed !== 'number' || Number.isNaN(params.extrusionSpeed)) return false;
    if (typeof params.matrixResolution !== 'number' || Number.isNaN(params.matrixResolution)) return false;
    if (typeof params.genomeLengthBp !== 'number' || Number.isNaN(params.genomeLengthBp)) return false;
    ...
    if (typeof results.steps !== 'number' || Number.isNaN(results.steps)) return false;
    if (typeof results.loopsFormed !== 'number' || Number.isNaN(results.loopsFormed)) return false;
    if (typeof results.convergentPairs !== 'number' || Number.isNaN(results.convergentPairs)) return false;
...
export function formatSummaryMarkdown(run: ArchcodeRun): string {
    ...
    const lines: string[] = [
        ...
        ...(res.uniqueLoopsFormed != null ? [`- **Unique pairs (left-right):** ${res.uniqueLoopsFormed}`] : []),
    ...
    if (res.stablePairs.length > 0) {
        ...
        res.stablePairs.slice(0, 3).forEach((p, i) => {
            lines.push(`${i + 1}. ${p.left.toLocaleString()} – ${p.right.toLocaleString()} bp${p.sizeBp != null ? ` (${(p.sizeBp / 1000).toFixed(1)} kb)` : ''}`);
        });
        lines.push('');
    }
...
export function buildRunInitial(params: {
    ...
    ctcfSites: Array<{ position: number; orientation: 'F' | 'R'; chrom?: string }>;
    seed?: number;
    chromosome?: string;
}): ArchcodeRun {
    ...
    return {
        ...
        model: {
            ctcfSites: params.ctcfSites.map(s => ({ pos: s.position, orient: s.orientation, chrom: s.chrom })),
            chromosome,
        },
        results: {
            steps: 0,
