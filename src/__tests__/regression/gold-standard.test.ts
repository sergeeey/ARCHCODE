...
const HBB_LOCUS: LocusConfig = {
  ...
  description:
    "Beta-globin locus - tests insulator function and convergent CTCF rule",
};
...
describe("Gold Standard Regression Tests", () => {
  ...
  describe("Sox2 Locus", () => {
    ...
    it("should demonstrate convergent rule: inversion changes loop pattern", () => {
      ...
      console.log(
        `WT loops: ${wtLoops.length}, Inverted loops: ${invLoops.length}`,
      );
  ...
  describe("Pcdh Locus", () => {
    ...
    it("should produce distinct TADs in contact matrix", () => {
      ...
      console.log(
        `Within-domain contact: ${withinAvg.toFixed(3)}, Between: ${betweenAvg.toFixed(3)}`,
      );
