...
describe('AlphaGenomeService strict-real contract', () => {
  ...
  afterEach(() => {
    ...
    if (originalViteApiKey === undefined) delete process.env.VITE_ALPHAGENOME_API_KEY;
  ...
  it('fails closed in strict-real mode without API key', async () => {
    ...
    await expect(service.predict(interval)).rejects.toThrow(/PR GATE FAILURE \[strict-real\]: AlphaGenome API key is missing/i);
  });
