...
console.log('CTCF sites: R@30kb, F@70kb (convergent: left leg hits R, right leg hits F)');
console.log('Cohesin loaded at 50kb');

const loops1 = engine1.run(100);
console.log(`Result: ${loops1.length} loop(s) formed`);
if (loops1.length > 0) {
    console.log(`✅ PASS: Loop anchors at ${loops1[0].leftAnchor/1000}kb - ${loops1[0].rightAnchor/1000}kb`);
} else {
...
const bedContent = fs.readFileSync('data/input/ctcf/GM12878_HBB_CTCF_peaks.bed', 'utf-8');
const bedResult = loadCTCFFromBED(bedContent);
console.log(`Parsed ${bedResult.parsed} CTCF sites`);
console.log(`Forward: ${bedResult.sites.filter(s => s.orientation === 'F').length}`);
console.log(`Reverse: ${bedResult.sites.filter(s => s.orientation === 'R').length}`);
...
loops3.forEach((loop, i) => {
    console.log(`  Loop ${i+1}: ${loop.leftAnchor.toLocaleString()} - ${loop.rightAnchor.toLocaleString()} bp (size: ${((loop.rightAnchor - loop.leftAnchor)/1000).toFixed(1)} kb)`);
});
...
if (loops3.length > 0) {
    ...
    console.log(`Power-law fit: α = ${fit.alpha.toFixed(3)}, R² = ${fit.r2.toFixed(3)}`);
