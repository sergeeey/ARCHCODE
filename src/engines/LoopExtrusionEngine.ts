...
/**
 * Convert loops to contact matrix
 */
export function loopsToContactMatrix(
    ...
    // Initialize matrix with background
    ...
    // Add distance-dependent background decay
    for (let i = 0; i < nBins; i++) {
    ...
    // Add loop-induced contacts
    for (const loop of loops) {
        // Guard: invalid loop coordinates
 ...
 * P(s) = probability of contact at genomic distance s
 */
export function computePSCurve(
    ...
    // Collect contacts at each distance
    for (let i = 0; i < nBins; i++) {
        // Guard: jagged matrix
    ...
    // Guard: no valid contacts collected
    ...
    const contacts = distances.map(d => {
        ...
        // Guard: empty values array (shouldn't happen, but be safe)
...
export function fitPSPowerLaw(
    ...
    // Filter out zero contacts (log undefined)
