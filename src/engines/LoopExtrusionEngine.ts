...
export class LoopExtrusionEngine {
  ...
  constructor(config: EngineConfig) {
    ...
    const loadPosition =
      config.cohesinLoadPosition ?? Math.floor(this.genomeLength / 2);
    ...
    console.log(
      `[Engine] Created with ${this.ctcfSites.length} CTCF sites, cohesin @ ${loadPosition}, genome: ${this.genomeLength}bp`,
    );
    console.log(
      `[Engine] CTCF positions:`,
      this.ctcfSites.map((s) => `${s.orientation}@${s.position}`).join(", "),
    );
  }
  ...
  private findNearestCTCF(
    ...
    if (direction === "left") {
      ...
      return validCandidates.reduce((max, s) =>
        s.position > max.position ? s : max,
      );
    } else {
      const validCandidates = candidates.filter((s) => s.position > position);
      if (validCandidates.length === 0) return null;
      return validCandidates.reduce((min, s) =>
  ...
  private isValidCohesinState(cohesin: CohesinComplex): boolean {
    if (
      !this.isFiniteNumber(cohesin.leftLeg) ||
      !this.isFiniteNumber(cohesin.rightLeg)
    ) {
      console.warn(
        `[Step ${this.stepCount}] Invalid cohesin state: non-finite legs`,
        cohesin,
      );
      return false;
    }
    if (cohesin.leftLeg >= cohesin.rightLeg) {
      console.warn(
        `[Step ${this.stepCount}] Invalid cohesin state: leftLeg >= rightLeg`,
        cohesin,
      );
      return false;
    }
  ...
  private checkBarriers(cohesin: CohesinComplex): boolean {
    ...
    if (leftBarrier || rightBarrier) {
      console.log(
        `[Step ${this.stepCount}] LEF @ ${cohesin.leftLeg}-${cohesin.rightLeg}`,
    ...
    if (leftBlocked && rightBlocked && leftBarrier && rightBarrier) {
      ...
      if (this.rng.random() < blockingEfficiency) {
        ...
        if (!this.isValidLoopAnchors(leftPos, rightPos)) {
          console.warn(
            `[Step ${this.stepCount}] Invalid loop anchors: ${leftPos}-${rightPos}`,
          );
          return false;
        }
        ...
        console.log(
          `[Step ${this.stepCount}] ✅ LOOP FORMED (eff=${blockingEfficiency}):`,
      ...
      console.log(
        `[Step ${this.stepCount}] ⚡ Leaky pass-through at convergent pair`,
      );
    }
  ...
  step(): boolean {
    ...
    if (this.stepCount % 100 === 0) {
      ...
      console.log(
        `[Step ${this.stepCount}] Active: ${activeCount}/${this.cohesins.length}, Loops: ${this.loops.length}`,
      );
    }
    ...
    if (this.cohesins.length > this.maxCohesins) {
      ...
      if (inactiveCount > 100) {
        ...
        console.log(
          `[Step ${this.stepCount}] Cleanup: removed ${removed} inactive cohesins`,
        );
      }
    }
    ...
    if (!hasActive && this.cohesins.length < this.maxCohesins) {
      if (this.loops.length === 0) {
        console.warn(
          `[Step ${this.stepCount}] ⚠️ All cohesins inactive, no loops formed!`,
        );
      }
