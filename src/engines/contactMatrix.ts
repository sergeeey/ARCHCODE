...
export function loopsToContactMatrix(
  ...
  if (genomeEnd <= genomeStart) {
    console.warn(
      "[loopsToContactMatrix] Invalid genome range:",
      genomeStart,
      "-",
      genomeEnd,
    );
  ...
  if (nBins > 10000) {
    console.warn(
      `[loopsToContactMatrix] Matrix too large (${nBins}x${nBins}), capping at 10000`,
    );
    return Array(10000)
      .fill(null)
      .map(() => Array(10000).fill(backgroundLevel));
  }

  const matrix: ContactMatrix = Array(nBins)
    .fill(null)
    .map(() => Array(nBins).fill(backgroundLevel));
...
export function fitPSPowerLaw(
  ...
  const predictions = logDistances.map(
    (ld) => meanLogC + alpha * (ld - meanLogD),
  );
  const ssRes = predictions.reduce(
    (sum, pred, i) => sum + (logContacts[i] - pred) ** 2,
    0,
  );
