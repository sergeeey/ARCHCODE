...
function analyzeMatrix(matrix: number[][], name: string): void {
  ...
  for (const p of diagonalProfiles) {
    console.log(
      `    ${p.distance.toString().padStart(2)} | ${p.mean.toFixed(4).padStart(7)} | ${p.std.toFixed(4).padStart(7)}`,
    );
  }
  ...
  for (const p of peaks.slice(0, 10)) {
    const dist = p.j - p.i;
    console.log(
      `    ${p.i.toString().padStart(3)} |   ${p.j.toString().padStart(3)} |      ${dist.toString().padStart(3)} | ${p.value.toFixed(4)}`,
    );
  }

  // Row/column sums (marginal distribution)
  const rowSums = matrix.map((row) => row.reduce((a, b) => a + b, 0) / n);
  const topRows = rowSums
    .map((s, i) => ({ bin: i, sum: s }))
  ...
  console.log(
    `  Non-zero entries: ${nonZero}/${n * n} (${((100 * nonZero) / (n * n)).toFixed(1)}%)`,
  );
}
...
function analyzeCorrelationByDistance(
  ...
  for (let d = 0; d < Math.min(20, n); d++) {
    ...
    console.log(
      `       ${d.toString().padStart(3)} |   ${r.toFixed(4).padStart(7)} |    ${values1.length.toString().padStart(4)}`,
    );
  }
}
...
async function main() {
  ...
  const apiKey =
    ...
    "";
  const alphaGenome = new AlphaGenomeService({
    apiKey,
