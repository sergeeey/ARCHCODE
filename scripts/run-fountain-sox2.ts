...
async function readMed1BigWig(bwPath: string): Promise<number[] | null> {
    try {
        ...
        console.log(`[SOX2] MED1 signal loaded: ${signalBins.filter(x => x > 0).length} non-zero bins`);
        return signalBins;
    } catch (e) {
...
async function main() {
    ...
    console.log('='.repeat(60));
    console.log(`Locus: ${SOX2_CHR}:${SOX2_START.toLocaleString()}-${SOX2_END.toLocaleString()}`);
    ...
    const ctcfSites = SOX2_CTCF_SITES.map(s => createCTCFSite(SOX2_CHR, s.pos, s.orient, s.strength));

    const nBins = Math.floor(SOX2_LENGTH / RESOLUTION);
    console.log(`[ensemble] beta=${beta}, runs=${numRuns}, steps=${MAX_STEPS}, bins=${nBins}`);
    console.log(`  stepLoadingProbability = ${fountain.getStepLoadingProbability().toFixed(6)}`);
    ...
    for (let run = 0; run < numRuns; run++) {
        ...
        if ((run + 1) % 5 === 0) {
            console.log(`  run ${run + 1}/${numRuns} done, loops so far: ${totalLoops}`);
        }
    }
