...
async function main() {
    ...
    const loci: Array<{ name: string; interval: GenomeInterval }> = customInterval
        ...
        : [
            { name: 'HBB (Beta-Globin)', interval: { chromosome: 'chr11', start: 5200000, end: 5400000 } },
            { name: 'MYC', interval: { chromosome: 'chr8', start: 127735000, end: 128835000 } },
            { name: 'SOX2', interval: { chromosome: 'chr3', start: 181500000, end: 182300000 } },
            { name: 'IGH', interval: { chromosome: 'chr14', start: 105586000, end: 106886000 } },
        ];
    ...
    const report: IntegrationReport = {
        ...
        parserPath,
        kramerParams: { alpha: 0.92, gamma: 0.80, kBase: 0.002 },
        analyses: [],
        summary: {
            totalAnalyses: 0,
            meanRiskScore: 0,
    ...
    for (const { name, interval } of loci) {
        ...
        try {
            ...
            console.log(`  ✓ Completed: ${analysis.loopsDetected} loops, risk=${analysis.riskScore}%`);
    ...
    console.log('|' + '─'.repeat(22) + '|' + '─'.repeat(7) + '|' + '─'.repeat(7) + '|' + '─'.repeat(7) + '|');
