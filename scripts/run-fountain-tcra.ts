...
async function readMed1BigWig(bwPath: string): Promise<number[] | null> {
    try {
        ...
        console.log(`[TCRα] MED1 signal loaded: ${signalBins.filter(x => x > 0).length} non-zero bins`);
        return signalBins;
    } catch (e) {
...
async function main() {
    ...
    console.log('='.repeat(60));
    console.log(`Locus: ${TCRA_CHR}:${TCRA_START.toLocaleString()}-${TCRA_END.toLocaleString()}`);
