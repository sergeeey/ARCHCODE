...
async function ensureDataFiles(): Promise<{
  med1: string | null;
  h3k27ac: string | null;
}> {
  console.log("Checking K562 data files...");

  const med1Ok = await downloadFile(
    ENCODE_FILES.MED1.url,
    ENCODE_FILES.MED1.localPath,
  );
  const h3k27acOk = await downloadFile(
    ENCODE_FILES.H3K27ac.url,
    ENCODE_FILES.H3K27ac.localPath,
  );
...
async function identifySuperEnhancers(
  h3k27acPath: string,
): Promise<SuperEnhancer[]> {
  console.log("");
  ...
  for (const [, info] of Object.entries(
  ...
  for (const chrom of MAIN_CHROMS) {
    ...
    for (let chunkStart = 0; chunkStart < chromSize; chunkStart += chunkSize) {
      ...
      try {
        const features = await bw.getFeatures(chrom, chunkStart, chunkEnd, {
          scale: 1,
        });
        ...
        const threshold =
          scores[
            Math.floor((scores.length * PEAK_THRESHOLD_PERCENTILE) / 100)
          ] || 0;
        ...
        let peakSignal =
          highSignal[0].score * (highSignal[0].end - highSignal[0].start);
  ...
  if (sorted.length > 0) {
    ...
    for (let i = 1; i < sorted.length; i++) {
      const peak = sorted[i];
      if (
        peak.chrom === current.chrom &&
        peak.start - current.end <= STITCH_DISTANCE
      ) {
...
async function validateSE(
  ...
  try {
    ...
    for (let i = 0; i < BIN_COUNT; i++) {
      ...
      try {
        const feats = await file.getFeatures(se.chrom, start, end, {
          scale: 1,
        });
        let sum = 0,
          n = 0;
  ...
  const ctcfSites = [
    ...
    createCTCFSite(
      se.chrom,
      Math.floor(seCenter - seHalfWidth * 0.3),
      "R",
      0.85,
    ),
    createCTCFSite(
    ...
    createCTCFSite(
      se.chrom,
      seRelativeEnd + Math.floor((windowLength - seRelativeEnd) * 0.4),
      "R",
      0.8,
    ),
    createCTCFSite(
      se.chrom,
      seRelativeEnd + Math.floor((windowLength - seRelativeEnd) * 0.8),
      "F",
      0.8,
    ),
  ];
  ...
  const occupancyMatrix: number[][] = Array(nBins)
    .fill(null)
  ...
  const contactDensityOutsideSE =
    cellsOutside > 0 ? contactsOutside / cellsOutside : 0;
  const contactDensityRatio =
    contactDensityOutsideSE > 0
      ? contactDensityInSE / contactDensityOutsideSE
      : 1;

  const avgLifetimeInSE =
    lifetimesInSE.length > 0
      ? lifetimesInSE.reduce((a, b) => a + b, 0) / lifetimesInSE.length
      : 0;
  const avgLifetimeOutsideSE =
    lifetimesOutside.length > 0
      ? lifetimesOutside.reduce((a, b) => a + b, 0) / lifetimesOutside.length
      : 0;
  const lifetimeRatio =
    avgLifetimeOutsideSE > 0 ? avgLifetimeInSE / avgLifetimeOutsideSE : 1;
...
async function main() {
  ...
  for (const se of superEnhancers) {
    ...
    bedLines.push(
      `${se.chrom}\t${se.start}\t${se.end}\t${se.name}\t${score}\t.\t${se.signal.toFixed(2)}\t${se.rank}`,
    );
  }
  ...
  const med1Path =
    dataFiles.med1 || dataFiles.h3k27ac || ENCODE_FILES.H3K27ac.localPath;
  console.log(
    `Using signal file: ${path.basename(med1Path)} (${dataFiles.med1 ? "MED1" : "H3K27ac proxy"})`,
  );
  ...
  for (let i = 0; i < topSE.length; i++) {
    ...
    process.stdout.write(
      `[${i + 1}/${topSE.length}] ${se.name} (${se.chrom}, ${seLen}kb)...`,
    );
    ...
    console.log(
      ` Contact: ${result.contactDensityRatio.toFixed(2)}x, Lifetime: ${result.lifetimeRatio.toFixed(2)}x`,
    );
  }

  // Calculate summary
  const avgContactRatio =
    results.reduce((s, r) => s + r.contactDensityRatio, 0) / results.length;
  const avgLifetimeRatio =
    results.reduce((s, r) => s + r.lifetimeRatio, 0) / results.length;
  ...
  console.log("");
  console.log(
    `${"Rank".padEnd(6)}${"SE Name".padEnd(16)}${"Chr".padEnd(8)}${"Contact↑".padEnd(12)}${"Lifetime↑".padEnd(12)}`,
  );
  console.log("-".repeat(70));
  ...
  console.log(
    "│" + "  CROSS-CELL COMPARISON: K562 vs GM12878".padEnd(68) + "│",
  );
  console.log("├" + "─".repeat(68) + "┤");
  console.log(
    "│" +
      `  K562 Mean Contact Enrichment:     ${avgContactRatio.toFixed(2)}x`.padEnd(
        68,
      ) +
      "│",
  );
  console.log(
    "│" +
      `  GM12878 Mean Contact Enrichment:  ${GM12878_CONTACT_RATIO.toFixed(2)}x`.padEnd(
        68,
      ) +
      "│",
  );
  ...
  console.log(
    "│" +
      `  K562 Mean Lifetime Ratio:         ${avgLifetimeRatio.toFixed(2)}x`.padEnd(
        68,
      ) +
      "│",
  );
  console.log(
    "│" +
      `  GM12878 Mean Lifetime Ratio:      ${GM12878_LIFETIME_RATIO.toFixed(2)}x`.padEnd(
        68,
      ) +
      "│",
  );
  ...
  const verdict = crossCellValid
    ? "✓ PASS — Mechanism is cell-type independent!"
    : "✗ FAIL";
  ...
  if (crossCellValid) {
    ...
    console.log(
      "║" +
        "  • High contact enrichment in SE zones (FountainLoader effect)".padEnd(
          68,
        ) +
        "║",
    );
    console.log(
      "║" +
        "  • Similar loop lifetime (~1.0x) regardless of cell type".padEnd(68) +
        "║",
    );
    console.log("║" + "".padEnd(68) + "║");
    console.log(
      "║" +
        "  This demonstrates that MED1-mediated cohesin loading is a".padEnd(
          68,
        ) +
        "║",
    );
    console.log(
      "║" +
        "  UNIVERSAL mechanism for super-enhancer architecture.".padEnd(68) +
        "║",
    );
    console.log("╚" + "═".repeat(68) + "╝");
  }
  ...
  const report = {
    ...
    comparison_gm12878: {
      ...
      contact_ratio_similar:
        Math.abs(avgContactRatio - GM12878_CONTACT_RATIO) /
          GM12878_CONTACT_RATIO <
        0.5,
      lifetime_ratio_similar:
        Math.abs(avgLifetimeRatio - GM12878_LIFETIME_RATIO) < 0.2,
    },
    results,
