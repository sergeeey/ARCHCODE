...
const DEFAULT_H3K27AC =
  "D:/ПАРСИНГ НАУЧНЫХ НОВОСТЕЙ/data/inputs/histone/H3K27ac_GM12878.bw";
const DEFAULT_MED1 =
...
async function processChromosome(
  bwPath: string,
  chrom: string,
  chromSize: number,
): Promise<Peak[]> {
  ...
  for (let chunkStart = 0; chunkStart < chromSize; chunkStart += chunkSize) {
    ...
    try {
      ...
      const thresholdIdx = Math.floor(
        (scores.length * PEAK_THRESHOLD_PERCENTILE) / 100,
      );
      const threshold = scores[thresholdIdx] || 0;

      // Collect high-signal features
      const highSignalFeatures: Array<{
        start: number;
        end: number;
        score: number;
      }> = [];
      ...
      let peakSignal =
        highSignalFeatures[0].score *
        (highSignalFeatures[0].end - highSignalFeatures[0].start);
...
async function main() {
  ...
  console.log(
    `After stitching (${STITCH_DISTANCE / 1000}kb): ${stitched.length} regions`,
  );
  ...
  for (const se of superEnhancers) {
    ...
    bedLines.push(
      `${se.chrom}\t${se.start}\t${se.end}\t${name}\t${score}\t.\t${se.signal.toFixed(2)}\t${se.rank}`,
    );
  }
  ...
  for (const e of enhancers) {
    ...
    allLines.push(
      `${e.chrom}\t${e.start}\t${e.end}\t${name}\t${score}\t.\t${e.signal.toFixed(2)}\t${e.type}\t${e.rank}`,
    );
  }
  ...
  console.log("═".repeat(75));
  console.log(
    `${"Rank".padEnd(6)}${"Chrom".padEnd(8)}${"Start".padEnd(12)}${"End".padEnd(12)}${"Length".padEnd(10)}${"Signal".padEnd(14)}`,
  );
  console.log("-".repeat(75));
  ...
  if (fs.existsSync(med1Path)) {
    ...
    for (const se of superEnhancers.slice(0, 10)) {
      try {
        const feats = await med1Bw.getFeatures(se.chrom, se.start, se.end, {
          scale: 1,
        });
        let sum = 0;
        ...
        console.log(
          `SE_${se.rank} (${se.chrom}:${se.start.toLocaleString()}-${se.end.toLocaleString()}): MED1 = ${sum.toFixed(2)}`,
        );
      } catch {
