...
async function generateFigure2A() {
  ...
  const wtAvg =
    wtLoops.length > 0
      ? wtLoops.reduce((s, l) => s + (l.rightAnchor - l.leftAnchor), 0) /
        wtLoops.length
      : 0;
  const invAvg =
    invLoops.length > 0
      ? invLoops.reduce((s, l) => s + (l.rightAnchor - l.leftAnchor), 0) /
        invLoops.length
      : 0;
...
async function generateSupplementaryParams() {
  ...
  for (const velocity of velocities) {
    for (const count of cohesinCounts) {
      ...
      console.log(
        `  Tested: v=${velocity}, n=${count}, r=${validation.pearsonCorrelation.toFixed(3)}`,
      );
    }
  }

  fs.writeFileSync(
    path.join(FIGURES_DIR, "supplementary_parameter_scan.csv"),
    csv,
  );
