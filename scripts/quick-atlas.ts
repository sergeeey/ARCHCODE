...
function main() {
  ...
  for (let i = 0; i < 367; i++) {
    ...
    const alphaPath =
      alphaVerdict === "Pathogenic" || alphaVerdict === "Likely Pathogenic";
  ...
  // Statistics
  const pathogenicArchcode = variants.filter(
    (v) =>
      v.archcodeVerdict === "PATHOGENIC" ||
      v.archcodeVerdict === "LIKELY_PATHOGENIC",
  ).length;
  const pathogenicAlpha = variants.filter(
    (v) =>
      v.alphaVerdict === "Pathogenic" || v.alphaVerdict === "Likely Pathogenic",
  ).length;
  const discordantCount = variants.filter((v) => v.discordant).length;
  const archcodeOnly = variants.filter(
    (v) =>
      v.discordant &&
      (v.archcodeVerdict === "PATHOGENIC" ||
        v.archcodeVerdict === "LIKELY_PATHOGENIC"),
  ).length;
  const alphaOnly = variants.filter(
    (v) =>
      v.discordant &&
      (v.alphaVerdict === "Pathogenic" ||
        v.alphaVerdict === "Likely Pathogenic"),
  ).length;
...
`;
  ...
  console.log(
    `ARCHCODE pathogenic: ${pathogenicArchcode} (${((pathogenicArchcode / 367) * 100).toFixed(1)}%)`,
  );
  console.log(
    `AlphaGenome pathogenic: ${pathogenicAlpha} (${((pathogenicAlpha / 367) * 100).toFixed(1)}%)`,
  );
  console.log(
    `Discordant: ${discordantCount} (${((discordantCount / 367) * 100).toFixed(1)}%)`,
  );
  ...
  loopThatStayed.slice(0, 3).forEach((v, i) => {
    ...
    console.log(
      `     SSIM: ${v.ssim.toFixed(3)} | Alpha: ${v.alphaScore.toFixed(3)}`,
    );
  });
}
