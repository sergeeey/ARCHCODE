...
import { AlphaGenomeClient, ValidationResult } from '../src/validation/alphagenome';
 ...
 */
function runSimulation(params: GridResult['params']): { loops: any[]; matrix: number[][] } {
        ...
        for (const processivity of PARAM_GRID.processivity) {
            for (const cohesinCount of PARAM_GRID.cohesinCount) {
                for (const stallProbability of PARAM_GRID.stallProbability) {
                    currentRun++;
                    const params = { velocity, processivity, cohesinCount, stallProbability };
                    
                    console.log(`[${currentRun}/${totalRuns}] Testing: v=${velocity}, n=${cohesinCount}`);
                    ...
                    try {
                        ...
                        const avgLoopSize = loops.length > 0 
                            ? loops.reduce((sum, l) => sum + (l.rightAnchor - l.leftAnchor), 0) / loops.length 
                            : 0;
                        ...
                        console.log(`   ✅ Pearson r = ${validation.pearsonCorrelation.toFixed(3)}, Loops: ${loops.length}`);
...
function findBestParams(results: GridResult[]): GridResult | null {
    ...
    const sorted = [...results].sort((a, b) => b.metrics.pearson - a.metrics.pearson);
    return sorted[0];
}
...
function printSummary(results: GridResult[]): void {
    ...
    const sorted = [...results].sort((a, b) => b.metrics.pearson - a.metrics.pearson).slice(0, 5);
