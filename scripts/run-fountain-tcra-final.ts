...
async function readMed1BigWig(bwPath: string): Promise<number[] | null> {
    try {
        ...
        for (let i = 0; i < BIN_COUNT; i++) {
            ...
            let sum = 0, n = 0;
            for (const f of feats) { sum += (f as { score?: number }).score ?? 0; n++; }
...
function constantSignal(n: number): number[] { return Array(n).fill(1); }
...
function calculateSEZoneEnrichment(matrix: number[][], seStartBin: number, seEndBin: number): number {
    let seSum = 0, seCount = 0;
    ...
    return outsideDensity > 0 ? seDensity / outsideDensity : (seDensity > 0 ? Infinity : 1);
}
...
async function runSimulation(beta: number, numRuns: number, signalBins: number[]): Promise<{
    ...
    const ctcfSites = TCRA_CTCF_SITES.map(s => createCTCFSite(TCRA_CHR, s.pos, s.orient, s.strength));
...
async function main() {
    ...
    console.log(`Coords:     ${TCRA_CHR}:${TCRA_START.toLocaleString()}-${TCRA_END.toLocaleString()}`);
    console.log(`Length:     ${(TCRA_LENGTH / 1_000_000).toFixed(2)} Mb`);
    console.log(`SE Zone:    ${SE_ZONE_START.toLocaleString()}-${SE_ZONE_END.toLocaleString()} (Eα enhancer)`);
    ...
    console.log(`  Step loading prob:  ${baseline.stepLoadingProbability.toExponential(2)}`);
    ...
    console.log(`  Step loading prob:  ${optimal.stepLoadingProbability.toExponential(2)}`);
    ...
    const seEnrichmentBaseline = calculateSEZoneEnrichment(baseline.occupancyMatrix, seStartBin, seEndBin);
    const seEnrichmentOptimal = calculateSEZoneEnrichment(optimal.occupancyMatrix, seStartBin, seEndBin);

    // Comparison
    const loadingIncrease = optimal.stepLoadingProbability / baseline.stepLoadingProbability;
    ...
    for (let i = 0; i < nBins; i++) {
        for (let j = 0; j < nBins; j++) {
            const diff = Math.abs((optimal.occupancyMatrix[i]?.[j] ?? 0) - (baseline.occupancyMatrix[i]?.[j] ?? 0));
    ...
    console.log('═'.repeat(70));
    console.log(`  Loading probability increase:  ${loadingIncrease.toFixed(1)}x`);
    console.log(`  Contact probability increase:  ${contactIncrease.toFixed(1)}x`);
    console.log(`  Differential cells:            ${diffCells}`);
    console.log('');
    console.log(`  SE Zone Enrichment (beta=0):   ${seEnrichmentBaseline.toFixed(2)}x`);
    console.log(`  SE Zone Enrichment (beta=${beta}):  ${seEnrichmentOptimal.toFixed(2)}x`);
    ...
    console.log('│' + `  VERDICT: ${verdict === 'PASS' ? '✓ ' : '✗ '}${verdict}`.padEnd(68) + '│');
    console.log('├' + '─'.repeat(68) + '┤');
    console.log('│' + `  Loading increase:           ${loadingIncrease.toFixed(1)}x (threshold: >2x)`.padEnd(68) + '│');
    console.log('│' + `  Differential cells:         ${diffCells} (threshold: >100)`.padEnd(68) + '│');
    console.log('│' + `  SE Zone enrichment:         ${seEnrichmentOptimal.toFixed(2)}x (vs ${seEnrichmentBaseline.toFixed(2)}x baseline)`.padEnd(68) + '│');
    console.log('└' + '─'.repeat(68) + '┘');
