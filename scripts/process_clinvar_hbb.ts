...
function categorizeVariant(
  ...
  // Nonsense (stop gain)
  if (
    c.includes("stop_gained") ||
    c.includes("nonsense") ||
    protein.includes("ter") ||
    protein.includes("*")
  ) {
    return "nonsense";
  }
  ...
  if (
    c.includes("missense") ||
    (protein && protein.length > 0 && !protein.includes("="))
  ) {
    return "missense";
  }
  ...
  // UTR
  if (
    c.includes("5_prime_utr") ||
    c.includes("5'_utr") ||
    coding.includes("5utr") ||
    coding.includes("-5'utr")
  ) {
    return "5_prime_UTR";
  }
  if (
    c.includes("3_prime_utr") ||
    c.includes("3'_utr") ||
    coding.includes("3utr") ||
    coding.includes("*")
  ) {
    return "3_prime_UTR";
  }
...
async function main() {
  ...
  // Filter pathogenic/likely pathogenic only
  const pathogenicVariants = rawVariants.filter(
    (v: any) =>
  ...
  console.log(
    `✅ Filtered to ${pathogenicVariants.length} pathogenic/likely pathogenic`,
  );
  ...
  const categoryCounts = processed.reduce(
    (acc, v) => {
  ...
  const csvHeader =
    "clinvar_id,chr,position,ref,alt,category,hgvs_c,hgvs_p,clinical_significance";
  ...
  console.log("\n═══════════════════════════════════════════");
  console.log(
    "Next step: python scripts/run_spliceai_hbb.py --variants data/hbb_real_variants.csv",
  );
  console.log("═══════════════════════════════════════════\n");
}
