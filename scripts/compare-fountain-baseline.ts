...
const BASELINE_PATH = path.join(
  ...
  "..",
  "results",
  "baseline_sabaté_v1.json",
);
const OUT_JSON = path.join(
  __dirname,
  "..",
  "results",
  "diff_map_fountain_vs_baseline.json",
);
const OUT_HTML = path.join(
  __dirname,
  "..",
  "results",
  "diff_map_fountain_vs_baseline.html",
);
...
function main(): void {
  ...
  if (rows !== B.length || cols !== (B[0]?.length ?? 0)) {
    throw new Error(
      `Matrix size mismatch: fountain ${rows}x${cols}, baseline ${B.length}x${B[0]?.length ?? 0}`,
    );
  }
  ...
  const payload = {
    description:
      "Diff Map: fountain_test_v1 (beta=2) minus baseline_sabaté_v1 (beta=0)",
    locus,
