...
async function readMed1Signal(
    ...
    try {
        ...
        for (let i = 0; i < binCount; i++) {
            ...
            try {
                const feats = await file.getFeatures(chr, binStart, binEnd, { scale: 1 });
                let sum = 0, n = 0;
...
async function runCondition(
    ...
    const effectiveSignal = condition === 'KO'
        ? Array(signalBins.length).fill(1)
        : signalBins;
    ...
    const seEnrichment = outsideCells > 0 && seCells > 0
...
async function main() {
    ...
    for (const [locusKey, locusConfig] of Object.entries(LOCI)) {
        ...
        console.log('');
        console.log('═'.repeat(70));
        console.log(`LOCUS: ${locusConfig.name} (${locusConfig.chr}:${locusConfig.start.toLocaleString()}-${locusConfig.end.toLocaleString()})`);
        console.log('═'.repeat(70));
        ...
        if (med1Available) {
            ...
            signalBins = Array(BIN_COUNT).fill(0).map((_, i) => {
                ...
                if (pos >= seStart && pos <= seEnd) {
                    return 5 + 3 * Math.sin((pos - seStart) / (seEnd - seStart) * Math.PI);
                }
        ...
        console.log(`  Loading prob:    ${wtResult.stepLoadingProb.toExponential(2)}`);
        ...
        console.log(`  Loading prob:    ${koResult.stepLoadingProb.toExponential(2)}`);
        console.log(`  Avg loops:       ${koResult.avgLoops.toFixed(1)}`);
        console.log(`  SE enrichment:   ${koResult.seEnrichment.toFixed(2)}x`);

        // Calculate comparison
        const contactLoss = wtResult.seZoneContact > 0
            ? ((wtResult.seZoneContact - koResult.seZoneContact) / wtResult.seZoneContact) * 100
            : 0;
        const enrichmentLoss = wtResult.seEnrichment > 0
            ? ((wtResult.seEnrichment - koResult.seEnrichment) / wtResult.seEnrichment) * 100
            : 0;
        const loopLoss = wtResult.avgLoops > 0
        ...
        console.log('├' + '─'.repeat(58) + '┤');
        console.log('│' + `  SE Contact Loss:       ${contactLoss.toFixed(1)}%`.padEnd(58) + '│');
        console.log('│' + `  SE Enrichment Loss:    ${enrichmentLoss.toFixed(1)}%`.padEnd(58) + '│');
        console.log('│' + `  Loop Frequency Loss:   ${loopLoss.toFixed(1)}%`.padEnd(58) + '│');
        console.log('└' + '─'.repeat(58) + '┘');
    }
    ...
    console.log('═'.repeat(70));
    console.log('');
    console.log(`${'Locus'.padEnd(10)}${'WT SE↑'.padEnd(12)}${'KO SE↑'.padEnd(12)}${'Contact Loss'.padEnd(15)}${'Enrichment Loss'.padEnd(15)}`);
    console.log('-'.repeat(70));
    ...
    // Average effects
    const avgContactLoss = comparisons.reduce((s, c) => s + c.contactLoss, 0) / comparisons.length;
    const avgEnrichmentLoss = comparisons.reduce((s, c) => s + c.enrichmentLoss, 0) / comparisons.length;

    console.log('');
    console.log('╔' + '═'.repeat(68) + '╗');
    console.log('║' + '  DEGRON VALIDATION RESULT'.padEnd(68) + '║');
    console.log('╠' + '═'.repeat(68) + '╣');
    console.log('║' + `  Mean SE Contact Loss:        ${avgContactLoss.toFixed(1)}%`.padEnd(68) + '║');
    console.log('║' + `  Mean SE Enrichment Loss:     ${avgEnrichmentLoss.toFixed(1)}%`.padEnd(68) + '║');
    console.log('╠' + '═'.repeat(68) + '╣');
    ...
    console.log('║' + '  Reference: Rinzema et al. - Mediator depletion reduces'.padEnd(68) + '║');
    console.log('║' + '  SE-specific contacts by 50-70% while maintaining CTCF loops'.padEnd(68) + '║');
    console.log('╚' + '═'.repeat(68) + '╝');
