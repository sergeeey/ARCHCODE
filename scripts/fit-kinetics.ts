 ...
 *
 * Fits alpha and gamma parameters to match literature-informed residence-time
 * targets derived from Sabaté et al., 2024 (bioRxiv, DOI:
 * 10.1101/2024.08.09.605990).
 *
...
import {
  MultiCohesinEngine,
  type KramerKineticsConfig,
} from "../src/engines/MultiCohesinEngine";
import { KRAMER_KINETICS } from "../src/domain/constants/biophysics";
import { SeededRandom } from "../src/utils/random";

// ============================================================================
// Literature-informed residence-time targets based on Sabaté et al., 2024
// (bioRxiv, DOI: 10.1101/2024.08.09.605990).
// These values are repository-side calibration targets, not raw FRAP
// measurement files bundled in this repo.
// ============================================================================
...
function calculateMSE(
  ...
  for (const d of data) {
    const predictedSteps = predictResidenceSteps(
      kBase,
      alpha,
      gamma,
      d.occupancy,
...
function gridSearch(
  ...
  const predictions = data.map((d) => ({
    ...
    predicted:
      predictResidenceSteps(kBase, bestAlpha, bestGamma, d.occupancy) / 60,
...
function resampleData(data: FRAPData[], rng: SeededRandom): FRAPData[] {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    const d = data[idx];
    const noisyTime =
      d.residenceTimeMin + rng.gaussian(0, d.residenceTimeStd / Math.sqrt(d.n));
    resampled.push({
      ...d,
...
function runBootstrap(
  ...
  for (let i = 0; i < nIterations; i++) {
    ...
    if ((i + 1) % 100 === 0) {
      ...
      process.stdout.write(
        `\r  Progress: ${i + 1}/${nIterations} (${rate.toFixed(1)}/s, ETA: ${eta.toFixed(0)}s)    `,
      );
    }
  }
  ...
  const stdAlpha = Math.sqrt(
    alphas.reduce((sum, a) => sum + Math.pow(a - meanAlpha, 2), 0) /
      (alphas.length - 1),
  );
  const stdGamma = Math.sqrt(
    gammas.reduce((sum, g) => sum + Math.pow(g - meanGamma, 2), 0) /
      (gammas.length - 1),
  );
...
async function validateWithSimulation(
  ...
  for (const frapData of FRAP_DATA) {
    ...
    const meanResidenceMin =
      residenceTimes.reduce((a, b) => a + b, 0) / residenceTimes.length;
...
async function main() {
  ...
  const bootstrapN = parseInt(
    args[args.indexOf("--bootstrap") + 1] || "1000",
    10,
  );
  ...
  console.log(
    "  FRAP Data: Sabaté et al., 2024 (bioRxiv, DOI: 10.1101/2024.08.09.605990)",
  );
  console.log("═".repeat(70));
  ...
  for (const p of fineResult.predictions) {
    ...
    console.log(
      `  │ ${p.condition.padEnd(14)} │ ${p.predicted.toFixed(1).padStart(7)} min │ ${p.actual.toFixed(1).padStart(7)} min │ ${(error >= 0 ? "+" : "") + error.toFixed(1).padStart(7)} min │`,
    );
  }
  console.log("  └────────────────┴───────────┴───────────┴───────────┘");
  ...
  if (doBootstrap) {
    ...
    console.log(
      `    α = ${bootstrap.meanAlpha.toFixed(4)} ± ${bootstrap.stdAlpha.toFixed(4)}`,
    );
    console.log(
      `        95% CI: [${bootstrap.ci95Alpha[0].toFixed(4)}, ${bootstrap.ci95Alpha[1].toFixed(4)}]`,
    );
    console.log(
      `    γ = ${bootstrap.meanGamma.toFixed(4)} ± ${bootstrap.stdGamma.toFixed(4)}`,
    );
    console.log(
      `        95% CI: [${bootstrap.ci95Gamma[0].toFixed(4)}, ${bootstrap.ci95Gamma[1].toFixed(4)}]`,
    );
  }
  ...
  if (doValidate) {
    ...
    for (const r of simResults) {
      console.log(
        `  │ ${r.condition.padEnd(14)} │ ${r.simulated.toFixed(1).padStart(7)} min │ ${r.expected.toFixed(1).padStart(7)} min │`,
      );
    }
    console.log("  └────────────────┴───────────┴───────────┘");
  }
