...
async function main() {
    ...
    const ctcfSites = MYC_CTCF_SITES.map(s => createCTCFSite(MYC_CHR, s.pos, s.orient, s.strength));
    ...
    console.log(`[ensemble] Written ${outPath} | avgLoops=${(totalLoops / numRuns).toFixed(1)}`);
}
