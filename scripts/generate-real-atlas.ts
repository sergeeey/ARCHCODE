...
function loadVepResults(): Map<string, VepResult> {
  ...
  if (!fs.existsSync(filePath)) {
    ...
    console.error(
      "Run: python scripts/run_vep_predictions.py --variants data/hbb_real_variants.csv",
    );
    process.exit(1);
  }
...
function simulateCohesin(
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    const unloadProb =
      kBase * (1 - alpha * Math.pow(Math.max(0.001, avgOcc), gamma));
...
async function main() {
  ...
  console.log(
    `  Matched: ${matchedVariants.length} variants have both ClinVar + VEP data`,
  );

  if (matchedVariants.length === 0) {
    console.error(
      "ERROR: No variants matched between ClinVar and VEP datasets",
    );
    process.exit(1);
  }
  ...
  const summary = {
    ...
    data_sources: {
      ...
      structural_predictor:
        "ARCHCODE loop extrusion simulation (Kramer kinetics)",
    },
    parameters: {
    ...
    statistics: {
      ...
      archcode_pathogenic_pct: parseFloat(
        ((archcodePathogenic.length / results.length) * 100).toFixed(1),
      ),
      discordant: discordant.length,
      discordant_pct: parseFloat(
        ((discordant.length / results.length) * 100).toFixed(1),
      ),
      archcode_only: archcodeOnly.length,
      vep_only: vepOnly.length,
      pearls: pearls.length,
      mean_ssim: parseFloat(
        (
          results.reduce((s, r) => s + r.ARCHCODE_SSIM, 0) / results.length
        ).toFixed(4),
      ),
      mean_vep_score: parseFloat(
        (results.reduce((s, r) => s + r.VEP_Score, 0) / results.length).toFixed(
          4,
        ),
      ),
    },
      ...
      [...new Set(results.map((r) => r.Category))].map((cat) => {
        ...
        return [
          cat,
          {
            ...
            mean_ssim: parseFloat(
              (
                catResults.reduce((s, r) => s + r.ARCHCODE_SSIM, 0) /
                catResults.length
              ).toFixed(4),
            ),
            pathogenic_count: catResults.filter(
              (r) =>
                r.ARCHCODE_Verdict === "PATHOGENIC" ||
                r.ARCHCODE_Verdict === "LIKELY_PATHOGENIC",
            ).length,
  ...
  console.log(
    `ARCHCODE pathogenic:     ${archcodePathogenic.length} (${summary.statistics.archcode_pathogenic_pct}%)`,
  );
  console.log(
    `Discordant:              ${discordant.length} (${summary.statistics.discordant_pct}%)`,
  );
  ...
  if (pearls.length > 0) {
    for (const p of pearls.slice(0, 10)) {
      ...
      console.log(
        `    SSIM=${p.ARCHCODE_SSIM.toFixed(3)} | VEP=${p.VEP_Score.toFixed(2)}`,
      );
    }
  } else {
  ...
  for (const [cat, info] of Object.entries(summary.category_breakdown)) {
    ...
    console.log(
      `  ${cat.padEnd(20)} n=${ci.count}, mean SSIM=${ci.mean_ssim}, pathogenic=${ci.pathogenic_count}`,
    );
  }
