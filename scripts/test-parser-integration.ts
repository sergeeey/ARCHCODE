...
async function main() {
    ...
    try {
        ...
        console.log(`\n✓ Import successful`);
        console.log(`  Interval: ${result.interval.chromosome}:${result.interval.start}-${result.interval.end}`);
        ...
        console.log(`\nSimulation Results:`);
        console.log(`  Contact Matrix: ${result.simulation.contactMatrix.length}×${result.simulation.contactMatrix[0]?.length || 0}`);
        ...
        if (result.simulation.loops.length > 0) {
            ...
            result.simulation.loops.slice(0, 5).forEach((loop, i) => {
                console.log(`    ${i + 1}. ${loop.start}-${loop.end} (strength: ${loop.strength.toFixed(3)})`);
            });
        }

        console.log(`\nEpigenetic Tracks:`);
        console.log(`  MED1: ${result.epigenetics.med1 ? `${result.epigenetics.med1.length} bins` : 'not available'}`);
        console.log(`  CTCF: ${result.epigenetics.ctcfBinding ? `${result.epigenetics.ctcfBinding.length} bins` : 'not available'}`);
    ...
    try {
        ...
        let sum = 0, count = 0, maxOffDiag = 0;
