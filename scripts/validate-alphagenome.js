...
const { LoopExtrusionEngine } = require('../dist/engines/LoopExtrusionEngine.js');
...
function parseInterval(str) {
    ...
    if (!match) {
        throw new Error(`Invalid interval format: ${str}. Expected: chr11:5200000-5350000`);
    }
    return {
        chromosome: match[1],
...
async function main() {
    ...
    if (!options.apiKey) {
        ...
        console.error('Set ALPHAGENOME_API_KEY environment variable or use --api-key');
        process.exit(1);
    }
    ...
    console.log(`Interval: ${interval.chromosome}:${interval.start.toLocaleString()}-${interval.end.toLocaleString()}`);
    ...
    const { loopsToContactMatrix } = require('../dist/engines/LoopExtrusionEngine.js');
    ...
    try {
        ...
        const pearsonColor = result.pearsonCorrelation > 0.8 ? '\x1b[32m' : 
                            result.pearsonCorrelation > 0.6 ? '\x1b[33m' : '\x1b[31m';
        const reset = '\x1b[0m';

        console.log(`Pearson Correlation:  ${pearsonColor}${result.pearsonCorrelation.toFixed(4)}${reset}`);
        ...
        // Save results
        const outputPath = path.join(__dirname, '..', 'data', 'output', `validation_${Date.now()}.json`);
