...
function main() {
    const beta0 = loadJson(path.join(__dirname, '..', 'results', 'ensemble_sox2_beta0.json'));
    const beta5 = loadJson(path.join(__dirname, '..', 'results', 'ensemble_sox2_beta5.json'));
    ...
    if (hasSignificantDiff) {
        console.log('✓ PASS: FountainLoader shows significant spatial signal effect on SOX2 locus');
    ...
    const outPath = path.join(__dirname, '..', 'results', 'diff_sox2_beta5_vs_beta0.json');
