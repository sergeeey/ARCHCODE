...
async function simulateVariant(
  ...
  const variantBin = Math.floor(
    (variant.position - HBB_LOCUS.start) / resolution,
  );
  ...
  const mutMatrix = await simulateWithKramer(
    nBins,
    variantBin,
    rng,
    variant.category,
...
async function simulateWithKramer(
  ...
  const effectStrength =
    variantCategory === "splice_site"
      ? 0.1 // Strong effect
      : variantCategory === "3_prime_UTR"
        ? 0.4 // Moderate effect
        : 0.6; // Weak effect
  ...
  const activeCTCF =
    disruptedBin !== null && variantCategory === "splice_site"
...
function calculateMatrixStats(
  ...
  const meanInsulation =
    insulationCount > 0 ? insulationSum / insulationCount : 0;
...
async function compareWithAlphaGenome(
  ...
  const archcodePathogenic =
    result.verdict === "LOOP_COLLAPSE" || result.verdict === "TAD_DISRUPTION";
  ...
  if (result.verdict === "LOOP_COLLAPSE" && alphaPathogenic) {
    ...
    interpretation =
      `CONVERGENT EVIDENCE: Both ARCHCODE (Loop Collapse, Δinsulation=${result.delta.insulation.toFixed(3)}) ` +
    ...
    evidenceStrength = "STRONG";
    interpretation =
      `CONVERGENT EVIDENCE: ARCHCODE shows TAD boundary disruption (Δintegrity=${result.delta.tadDisruption.toFixed(3)}), ` +
    ...
    if (isUTR && !archcodePathogenic && alphaPathogenic) {
      ...
      interpretation =
        `MECHANISM INSIGHT: ARCHCODE (${result.verdict}) shows NO 3D structure disruption, ` +
      ...
      evidenceStrength = "MODERATE";
      interpretation =
        `MECHANISM INSIGHT: Splice site variant ${result.variant.clinvar_id} shows minimal 3D impact. ` +
      ...
      evidenceStrength = "MODERATE";
      interpretation =
        `DISCORDANT: ARCHCODE (${result.verdict}) and AlphaGenome (${alphagenomePrediction}) disagree. ` +
        `Requires additional experimental validation.`;
    }
  } else {
    evidenceStrength = "WEAK";
    interpretation =
      `CONCORDANT BENIGN: Both methods suggest limited pathogenic potential. ` +
...
function generateReport(
  ...
  const loopCollapse = results.filter(
    (r) => r.verdict === "LOOP_COLLAPSE",
  ).length;
  const tadDisruption = results.filter(
    (r) => r.verdict === "TAD_DISRUPTION",
  ).length;
  const mildEffect = results.filter((r) => r.verdict === "MILD_EFFECT").length;
  const benign = results.filter((r) => r.verdict === "BENIGN").length;
  const strongEvidence = comparisons.filter(
    (c) => c.evidenceStrength === "STRONG",
  ).length;
  ...
  const spliceVariants = variants.filter((v) =>
    v.category.includes("splice"),
  ).length;
...
${
  variants
    .filter((v) => v.evidenceStrength === "STRONG")
    .map((v) => `- **${v.clinvar_id}**: ${v.interpretation}`)
    .join("\n") ||
...
---
*Generated by ARCHCODE v1.1.0 | Kramer kinetics (α=0.92, γ=0.80) validated against literature ranges (Sabaté et al., 2024, bioRxiv)*
...
async function main() {
  ...
  for (const variant of PRIORITY_VUS) {
    ...
    console.log(
      `  Category: ${variant.category} | Mystery Score: ${variant.mystery_score}`,
    );
    ...
    console.log(
      `  ARCHCODE: ${result.verdict} (confidence: ${(result.confidence * 100).toFixed(0)}%)`,
    );
  ...
  console.log(
    `Concordance rate: ${(report.summary.concordanceRate * 100).toFixed(0)}%`,
  );
  console.log(`\n${report.conclusion}`);
