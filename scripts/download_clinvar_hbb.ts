...
async function downloadClinVarVariants(): Promise<ClinVarVariant[]> {
  ...
  console.log(
    `✅ Found ${variantIds.length} pathogenic/likely pathogenic HBB variants`,
  );

  if (variantIds.length === 0) {
    console.error(
      "❌ No variants found. Check API or use VCF download instead.",
    );
    process.exit(1);
  }
  ...
  for (let i = 0; i < variantIds.length; i += batchSize) {
    ...
    console.log(
      `📥 Downloading batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(variantIds.length / batchSize)}...`,
    );
...
function parseVariantSummary(data: any): ClinVarVariant | null {
  try {
    ...
    const vcvAccession =
      data.accession || `VCV${String(data.uid).padStart(9, "0")}`;
    ...
    const position = parseInt(
      grch38Loc.start || grch38Loc.display_start || "0",
      10,
    );
...
async function main() {
  ...
  console.log(
    `Locus: ${HBB_GENE.chromosome}:${HBB_GENE.start}-${HBB_GENE.end}`,
  );
