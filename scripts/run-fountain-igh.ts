...
async function readMed1BigWig(bwPath: string): Promise<number[] | null> {
    try {
        ...
        console.log(`[IGH] MED1 signal loaded: ${signalBins.filter(x => x > 0).length} non-zero bins`);
        return signalBins;
    } catch (e) {
...
async function main() {
    ...
    console.log('='.repeat(60));
    console.log(`Locus: ${IGH_CHR}:${IGH_START.toLocaleString()}-${IGH_END.toLocaleString()}`);
    ...
    const ctcfSites = IGH_CTCF_SITES.map(s => createCTCFSite(IGH_CHR, s.pos, s.orient, s.strength));
