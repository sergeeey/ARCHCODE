...
const SE_BED_PATH = path.join(
  __dirname,
  "..",
  "results",
  "super_enhancers_GM12878.bed",
);
const MED1_BW_PATH =
...
async function runSEValidation(
  ...
  for (let run = 0; run < NUM_RUNS; run++) {
    ...
    for (const loop of loops) {
      ...
      const isInSE =
        loopMidpoint >= seRelativeStart && loopMidpoint <= seRelativeEnd;
  ...
  const avgLifetimeInSE =
    lifetimeStats.inSE.length > 0
      ? lifetimeStats.inSE.reduce((a, b) => a + b, 0) /
        lifetimeStats.inSE.length
      : 0;
  const avgLifetimeOutsideSE =
    lifetimeStats.outsideSE.length > 0
      ? lifetimeStats.outsideSE.reduce((a, b) => a + b, 0) /
        lifetimeStats.outsideSE.length
      : 0;

  const lifetimeRatio =
    avgLifetimeOutsideSE > 0
...
async function main() {
  ...
  for (let i = 0; i < topSE.length; i++) {
    ...
    process.stdout.write(
      `[${i + 1}/${topSE.length}] ${se.name} (${se.chrom}:${se.start.toLocaleString()}, ${seLength.toFixed(1)}kb)...`,
    );
  ...
  const avgLifetimeRatio =
    results.reduce((sum, r) => sum + r.lifetimeRatio, 0) / results.length;
  const avgContactRatio =
    results.reduce((sum, r) => sum + r.contactDensityRatio, 0) / results.length;
  const totalLoopsInSE = results.reduce((sum, r) => sum + r.loopsInSE, 0);
  const totalLoopsOutside = results.reduce(
    (sum, r) => sum + r.loopsOutsideSE,
    0,
  );
  const avgLifetimeInSE =
    results.reduce((sum, r) => sum + r.avgLifetimeInSE, 0) / results.length;
  const avgLifetimeOutside =
    results.reduce((sum, r) => sum + r.avgLifetimeOutsideSE, 0) /
    results.length;
  const avgContactInSE =
    results.reduce((sum, r) => sum + r.contactDensityInSE, 0) / results.length;
  const avgContactOutside =
    results.reduce((sum, r) => sum + r.contactDensityOutsideSE, 0) /
    results.length;
  ...
  console.log(
    "SUMMARY: Loop Lifetime & Contact Density in SE vs Regular Regions",
  );
  console.log("═".repeat(85));
  console.log("");
  console.log(
    `${"Rank".padEnd(6)}${"Name".padEnd(16)}${"Chrom".padEnd(8)}${"SE Length".padEnd(10)}${"Contact↑".padEnd(12)}${"Lifetime↑".padEnd(12)}${"Status".padEnd(10)}`,
  );
  console.log("-".repeat(85));

  for (const r of results) {
    const seLength =
      ((r.seRelativeEnd - r.seRelativeStart) / 1000).toFixed(1) + "kb";
    const status =
      r.contactDensityRatio > 1.2
        ? "✓"
        : r.contactDensityRatio > 1.0
          ? "~"
          : "✗";
    console.log(
  ...
  console.log(
    "│" +
      `  Super-Enhancers validated:              ${results.length}`.padEnd(73) +
      "│",
  );
  console.log(
    "│" +
      `  Total loops in SE zones:                ${totalLoopsInSE}`.padEnd(73) +
      "│",
  );
  console.log(
    "│" +
      `  Total loops outside SE:                 ${totalLoopsOutside}`.padEnd(
        73,
      ) +
      "│",
  );
  ...
  console.log(
    "│" +
      `  Avg contact density in SE:              ${avgContactInSE.toFixed(4)} /kb/step`.padEnd(
        73,
      ) +
      "│",
  );
  console.log(
    "│" +
      `  Avg contact density outside SE:         ${avgContactOutside.toFixed(4)} /kb/step`.padEnd(
        73,
      ) +
      "│",
  );
  console.log(
    "│" +
      `  MEAN CONTACT DENSITY RATIO (SE/out):    ${avgContactRatio.toFixed(2)}x`.padEnd(
        73,
      ) +
      "│",
  );
  ...
  console.log(
    "│" +
      `  Avg loop lifetime in SE:                ${avgLifetimeInSE.toFixed(1)} ticks`.padEnd(
        73,
      ) +
      "│",
  );
  console.log(
    "│" +
      `  Avg loop lifetime outside SE:           ${avgLifetimeOutside.toFixed(1)} ticks`.padEnd(
        73,
      ) +
      "│",
  );
  console.log(
    "│" +
      `  Mean lifetime ratio (SE/outside):       ${avgLifetimeRatio.toFixed(2)}x`.padEnd(
        73,
      ) +
      "│",
  );
  ...
  const verdict =
    avgContactRatio > 1.2 ? "✓ PASS (Contact enrichment)" : "✗ FAIL";
  ...
  console.log("");
  console.log(
    "NOTE: Loop lifetime is expected to be similar (~1.0x) because FountainLoader",
  );
