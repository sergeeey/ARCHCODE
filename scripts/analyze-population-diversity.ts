...
function generatePopulationProfiles(rng: SeededRandom): SampleProfile[] {
    ...
    const variants = [
        // Normal variants (5)
        { type: 'normal', med1Factor: [0.9, 1.1], ctcfFactor: [0.95, 1.0], count: 5 },
        ...
        { type: 'severe', med1Factor: [0.2, 0.5], ctcfFactor: [0.3, 0.6], count: 6 },
    ];

    let sampleIdx = 1;
    for (const variant of variants) {
        for (let i = 0; i < variant.count; i++) {
            const med1Factor = rng.randomFloat(variant.med1Factor[0], variant.med1Factor[1]);
            const ctcfFactor = rng.randomFloat(variant.ctcfFactor[0], variant.ctcfFactor[1]);
            ...
            const phenotypeLabel = phenotype === 'normal' ? 'Normal' :
                                   phenotype === 'mild' ? 'Mild variant' : 'Severe variant';
...
function calculateSSIM(img1: number[][], img2: number[][]): number {
    ...
    for (let i = 0; i < n; i++) {
        for (let j = 0; j < Math.min(img1[i]?.length ?? 0, img2[i]?.length ?? 0); j++) {
...
function runSimulation(profile: SampleProfile): number[][] {
    ...
    const betaMultiplier = profile.phenotype === 'severe' ? 0.3 :
                          profile.phenotype === 'mild' ? 0.7 : 1.0;
    ...
    const meanOccupancy = profile.med1Occupancy.reduce((a, b) => a + b, 0) / profile.med1Occupancy.length;
...
function calculateInsulation(matrix: number[][], halfWindow: number = 2): number {
...
async function main() {
    ...
    console.log(`    - Normal: ${profiles.filter(p => p.phenotype === 'normal').length}`);
    console.log(`    - Mild variants: ${profiles.filter(p => p.phenotype === 'mild').length}`);
    console.log(`    - Severe variants: ${profiles.filter(p => p.phenotype === 'severe').length}`);
    ...
    for (let i = 0; i < profiles.length; i++) {
        ...
        process.stdout.write(`\r  [${i + 1}/${profiles.length}] ${profile.id}: ${profile.name.padEnd(30)}`);
    ...
    console.log('');
    console.log('┌──────┬────────────────────────────────┬──────────┬────────┬────────────┐');
    console.log('│  ID  │             Name               │ Phenotype│  SSIM  │ Risk Score │');
    console.log('├──────┼────────────────────────────────┼──────────┼────────┼────────────┤');

    for (const r of results) {
        const phenoColor = r.phenotype === 'normal' ? '🟢' :
                          r.phenotype === 'mild' ? '🟡' : '🔴';
        const riskBar = '█'.repeat(Math.min(10, Math.floor(r.riskScore / 10)));
        console.log(`│ ${r.id.padEnd(4)} │ ${r.name.padEnd(30)} │ ${phenoColor} ${r.phenotype.padEnd(6)} │ ${r.ssim.toFixed(3).padStart(6)} │ ${String(r.riskScore).padStart(3)}% ${riskBar.padEnd(5)} │`);
    }
    console.log('└──────┴────────────────────────────────┴──────────┴────────┴────────────┘');
    ...
    const meanRisk = results.reduce((sum, r) => sum + r.riskScore, 0) / results.length;
    const stdRisk = Math.sqrt(results.reduce((sum, r) => sum + Math.pow(r.riskScore - meanRisk, 2), 0) / results.length);
    ...
    const reportPath = path.join(RESULTS_DIR, 'population_diversity_analysis.json');
    ...
    console.log('╔═══════════════════════════════════════════════════════════════════╗');
    if (highRiskCount > 5) {
        console.log('║  ⚠️  HIGH GENETIC VARIABILITY DETECTED                            ║');
        console.log(`║  ${highRiskCount} samples show significant structural deviation           ║`);
    } else {
        console.log('║  ✅ POPULATION WITHIN NORMAL RANGE                                ║');
        console.log(`║  Only ${highRiskCount} samples show elevated risk                          ║`);
    }
    console.log('╚═══════════════════════════════════════════════════════════════════╝');
}
