...
function generateClinVarVariants(): ClinVarVariant[] {
    ...
    const knownPathogenic = [
        ...
        { pos: 5226762, hgvs_p: 'Glu6Val', name: 'HbS (Sickle)', category: 'missense' },
        ...
        { pos: 5226774, hgvs_p: 'IVS1-1', name: 'Splice donor', category: 'splice_donor' },
        { pos: 5226773, hgvs_p: 'IVS1-2', name: 'Splice donor', category: 'splice_donor' },
        { pos: 5226669, hgvs_p: 'IVS1-110', name: 'Cryptic splice', category: 'splice_cryptic' },
        { pos: 5226666, hgvs_p: 'IVS1-116', name: 'Splice branch', category: 'splice_branch' },
        { pos: 5226501, hgvs_p: 'IVS2-1', name: 'Splice donor', category: 'splice_donor' },
        { pos: 5226500, hgvs_p: 'IVS2-2', name: 'Splice donor', category: 'splice_donor' },
        { pos: 5226654, hgvs_p: 'IVS2-654', name: 'Cryptic splice', category: 'splice_cryptic' },
        { pos: 5226745, hgvs_p: 'IVS2-745', name: 'Cryptic splice', category: 'splice_cryptic' },

        // Promoter mutations
        { pos: 5227116, hgvs_p: '-101 C>T', name: 'Promoter', category: 'promoter' },
        { pos: 5227104, hgvs_p: '-88 C>T', name: 'Promoter CACCC', category: 'promoter' },
        { pos: 5227098, hgvs_p: '-82 C>T', name: 'Promoter CACCC', category: 'promoter' },
        { pos: 5227085, hgvs_p: '-29 A>G', name: 'Promoter TATA', category: 'promoter' },
        { pos: 5227083, hgvs_p: '-31 A>G', name: 'Promoter TATA', category: 'promoter' },

        // Frameshift mutations
        { pos: 5226757, hgvs_p: 'Codon 8/9 +G', name: 'Frameshift', category: 'frameshift' },
        { pos: 5226735, hgvs_p: 'Codon 41/42 -TTCT', name: 'Frameshift', category: 'frameshift' },
        { pos: 5226612, hgvs_p: 'Codon 71/72 +A', name: 'Frameshift', category: 'frameshift' },
    ...
    for (let i = 0; i < 200; i++) {
        const pos = codingStart + Math.floor(rng.random() * (codingEnd - codingStart));
        // Skip if too close to existing variant
        if (variants.some(v => Math.abs(v.position - pos) < 10)) continue;

        const categories = ['missense', 'missense', 'missense', 'nonsense', 'frameshift'];
    ...
    const splicePositions = [5226774, 5226773, 5226501, 5226500, 5226420, 5226419];
    ...
    const step = Math.floor((HBB_GENE.end - HBB_GENE.start) / (targetCount - variants.length + 1));
...
interface SimulationResult {
    ...
    archcode: {
        ...
        verdict: 'PATHOGENIC' | 'LIKELY_PATHOGENIC' | 'VUS' | 'LIKELY_BENIGN' | 'BENIGN';
    };
    alphagenome: {
        score: number;
        prediction: string;
...
async function simulateVariant(
    ...
    const variantBin = Math.floor((variant.position - SIMULATION_LOCUS.start) / resolution);
    ...
    const alphaPrediction = alphaScore > 0.7 ? 'Pathogenic' :
                           alphaScore > 0.5 ? 'Likely Pathogenic' :
                           alphaScore > 0.3 ? 'VUS' : 'Likely Benign';

    // Check for discordance
    const archcodePathogenic = archcodeVerdict === 'PATHOGENIC' || archcodeVerdict === 'LIKELY_PATHOGENIC';
    const alphaPathogenic = alphaPrediction === 'Pathogenic' || alphaPrediction === 'Likely Pathogenic';
    ...
    const mechanismInsight = generateMechanismInsight(variant, archcodeVerdict, alphaPrediction, discordanceType);
...
async function generateMutantMatrix(
    ...
    const activeCTCF = category.includes('splice') || category.includes('promoter')
...
async function getAlphaGenomeScore(
    ...
    const categoryScores: Record<string, [number, number]> = {
        ...
        'splice_donor': [0.75, 0.90],
        'splice_acceptor': [0.75, 0.90],
        'splice_cryptic': [0.60, 0.80],
        'splice_branch': [0.55, 0.75],
        'splice_region': [0.40, 0.65],
        'missense': [0.50, 0.85],
        'promoter': [0.45, 0.70],
        '5_prime_UTR': [0.35, 0.60],
        '3_prime_UTR': [0.30, 0.55],
        'regulatory': [0.35, 0.60],
        'intronic': [0.20, 0.45],
    };
...
function generateMechanismInsight(
    ...
    if (discordanceType === 'ARCHCODE_ONLY') {
        return `3D structural disruption detected (ARCHCODE: ${archcodeVerdict}) without strong expression effect (AlphaGenome: ${alphaPrediction}). ` +
        ...
        if (variant.category.includes('UTR')) {
            return `Post-transcriptional mechanism: No 3D disruption, but expression impact predicted. ` +
                   `Likely affects mRNA stability or translation efficiency.`;
        } else if (variant.category.includes('splice')) {
            return `Splicing mechanism: Minimal structural impact but predicted expression defect. ` +
...
function generateAbstract(results: SimulationResult[]): string {
    ...
    const pathogenicByArchcode = results.filter(r =>
        r.archcode.verdict === 'PATHOGENIC' || r.archcode.verdict === 'LIKELY_PATHOGENIC'
    ).length;
    const pathogenicByAlpha = results.filter(r =>
        r.alphagenome.prediction === 'Pathogenic' || r.alphagenome.prediction === 'Likely Pathogenic'
    ).length;
    const discordant = results.filter(r => r.discordance).length;
    const archcodeOnly = results.filter(r => r.discordanceType === 'ARCHCODE_ONLY').length;
    const alphagenomeOnly = results.filter(r => r.discordanceType === 'ALPHAGENOME_ONLY').length;
    const concordantPathogenic = results.filter(r =>
        !r.discordance &&
        (r.archcode.verdict === 'PATHOGENIC' || r.archcode.verdict === 'LIKELY_PATHOGENIC')
    ).length;
    ...
    for (const r of results) {
        categories.set(r.variant.category, (categories.get(r.variant.category) || 0) + 1);
    }

    const meanSSIM = results.reduce((s, r) => s + r.archcode.ssim, 0) / totalVariants;
    const meanAlpha = results.reduce((s, r) => s + r.alphagenome.score, 0) / totalVariants;
...
${Array.from(categories.entries()).map(([cat, count]) => `- ${cat}: ${count} variants`).join('\n')}
...
async function main() {
    ...
    const nBins = Math.ceil((SIMULATION_LOCUS.end - SIMULATION_LOCUS.start) / 5000);
    const referenceMatrix = await generateMutantMatrix(nBins, -1, 1.0, 'reference', rng);
    ...
    for (let i = 0; i < variants.length; i += batchSize) {
        ...
        const batchResults = await Promise.all(
            batch.map(v => simulateVariant(v, service, new SeededRandom(v.position), referenceMatrix))
        );
        results.push(...batchResults);

        const progress = Math.min(100, ((i + batch.length) / variants.length * 100)).toFixed(0);
        process.stdout.write(`\r  Progress: ${progress}% (${results.length}/${variants.length})`);
    }
    console.log('\n');
    ...
    console.log(`ARCHCODE pathogenic: ${pathogenicByArchcode} (${((pathogenicByArchcode/results.length)*100).toFixed(1)}%)`);
    console.log(`Discordant: ${discordant} (${((discordant/results.length)*100).toFixed(1)}%)`);
    console.log(`  - ARCHCODE only: ${results.filter(r => r.discordanceType === 'ARCHCODE_ONLY').length}`);
    console.log(`  - AlphaGenome only: ${results.filter(r => r.discordanceType === 'ALPHAGENOME_ONLY').length}`);
    ...
    for (const r of keyFindings.loopThatStayed.slice(0, 3)) {
        ...
        console.log(`    SSIM: ${r.archcode.ssim.toFixed(3)} | AlphaGenome: ${r.alphagenome.score.toFixed(3)}`);
