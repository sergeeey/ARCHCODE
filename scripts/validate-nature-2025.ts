...
import { createCTCFSite, Loop, getLoopDurationSteps } from '../src/domain/models/genome';
...
const locusKey = (args.values.locus ?? 'HBB').toUpperCase() as keyof typeof LOCI;
...
function histogram(durationsSteps: number[], binMinutes: number): Map<number, number> {
...
function main() {
    ...
    for (let i = 0; i < NUM_RUNS; i++) {
        ...
        if ((i + 1) % 100 === 0) {
            console.log = log;
            log(`  Run ${i + 1}/${NUM_RUNS} — loops with duration so far: ${allDurationsSteps.length}`);
            console.log = () => {};
        }
    }
    ...
    const contactProb = totalStepSeconds > 0 ? contactTimeSum / totalStepSeconds : 0;

    const targetMin = SABATE_NATURE_2025.LOOP_DURATION_TARGET_MIN_STEPS / STEPS_PER_MINUTE;
    const targetMax = SABATE_NATURE_2025.LOOP_DURATION_TARGET_MAX_STEPS / STEPS_PER_MINUTE;
...
`;
    ...
    const outPath = path.join(__dirname, '..', 'results', 'blind_test_validation_2025.md');
    fs.mkdirSync(path.dirname(outPath), { recursive: true });
    fs.writeFileSync(outPath, report, 'utf8');

    console.log('\n--- Summary ---');
    console.log(`Mean loop duration: ${meanMin.toFixed(2)} min (target 6–19 min)`);
    console.log(`95% CI: [${ci95LowerMin.toFixed(2)}, ${ci95UpperMin.toFixed(2)}] min`);
