...
function computeDiffStats(
  beta5: number[][],
  beta0: number[][],
): { diffCells: number; maxEnrichment: number; meanEnrichment: number } {
  let diffCells = 0,
    maxDiff = 0,
    sumDiff = 0,
    count = 0;
...
function analyzeLocus(name: keyof typeof LOCUS_CONFIG): LocusResult | null {
  ...
  const beta0StepLoad =
    beta0Data.fountainLoader?.stepLoadingProbability ??
    beta0Data.stepLoadingProbability ??
    0;
  const beta5StepLoad =
    beta5Data.fountainLoader?.stepLoadingProbability ??
    beta5Data.stepLoadingProbability ??
    0;

  const diffStats = computeDiffStats(beta5Heatmap, beta0Heatmap);
  const seEnrichment = computeSEZoneEnrichment(
    beta5Heatmap,
    beta0Heatmap,
    config.seZone,
  );
...
function generateMarkdownReport(report: FinalReport): string {
  ...
  lines.push(
    `| **Loci Tested** | ${report.loci.length} (MYC + 3 blind tests) |`,
  );
  lines.push(
    `| **Mean Loading Increase** | **${report.globalStats.meanLoadingFold.toFixed(1)}x** |`,
  );
  lines.push(
    `| **Mean SE Zone Enrichment** | **${report.globalStats.meanSEEnrichment.toFixed(1)}x** |`,
  );
  lines.push(
    `| **Total Differential Cells** | ${report.globalStats.totalDiffCells.toLocaleString()} |`,
  );
  lines.push(
    `| **Overall Verdict** | **${report.globalStats.overallVerdict}** |`,
  );
  lines.push("");
  ...
  lines.push(
    `║  probability in super-enhancer zones by ${report.globalStats.meanSEEnrichment.toFixed(1)}x compared to            ║`,
  );
  lines.push(
    `║  uniform loading (beta=0), validated across ${report.loci.length} independent loci.       ║`,
  );
  lines.push(
    "╚══════════════════════════════════════════════════════════════════════╝",
  );
  ...
  for (const locus of report.loci) {
    ...
    lines.push("");
    lines.push(
      `**Coordinates:** \`${locus.chromosome}:${locus.start.toLocaleString()}-${locus.end.toLocaleString()}\``,
    );
    lines.push("");
    lines.push("| Parameter | Baseline (β=0) | Fountain (β=5) | Change |");
    lines.push("|-----------|----------------|----------------|--------|");
    lines.push(
      `| Step Loading Prob | ${locus.beta0.stepLoadingProb.toExponential(2)} | ${locus.beta5.stepLoadingProb.toExponential(2)} | ${locus.enrichment.loadingFold.toFixed(1)}x |`,
    );
    lines.push(
      `| Avg Loops/Run | ${locus.beta0.avgLoops.toFixed(1)} | ${locus.beta5.avgLoops.toFixed(1)} | +${(locus.beta5.avgLoops - locus.beta0.avgLoops).toFixed(1)} |`,
    );
    lines.push(
      `| Non-Zero Cells | ${locus.beta0.nonZeroCells} | ${locus.beta5.nonZeroCells} | +${locus.beta5.nonZeroCells - locus.beta0.nonZeroCells} |`,
    );
    lines.push(
      `| Max Contact | ${locus.beta0.maxContact.toExponential(2)} | ${locus.beta5.maxContact.toExponential(2)} | ${(locus.beta5.maxContact / locus.beta0.maxContact).toFixed(1)}x |`,
    );
    lines.push("");
  ...
  lines.push("├────────────────────────────────────────────────────────────┤");
  lines.push(
    `│  Loci analyzed:              ${report.loci.length.toString().padStart(28)} │`,
  );
  lines.push(
    `│  Mean loading fold-change:   ${report.globalStats.meanLoadingFold.toFixed(2).padStart(25)}x │`,
  );
  lines.push(
    `│  Mean contact fold-change:   ${report.globalStats.meanContactFold.toFixed(2).padStart(25)}x │`,
  );
  lines.push(
    `│  Mean SE zone enrichment:    ${report.globalStats.meanSEEnrichment.toFixed(2).padStart(25)}x │`,
  );
  lines.push(
    `│  Total differential cells:   ${report.globalStats.totalDiffCells.toString().padStart(26)} │`,
  );
  lines.push("├────────────────────────────────────────────────────────────┤");
  lines.push(
    `│  VERDICT:                    ${report.globalStats.overallVerdict.padStart(26)} │`,
  );
  lines.push("└────────────────────────────────────────────────────────────┘");
  lines.push("```");
  lines.push("");
  ...
  lines.push("");
  lines.push(
    `3. **Quantitative Effect:** Average ${report.globalStats.meanSEEnrichment.toFixed(1)}x enrichment in super-enhancer zones.`,
  );
  lines.push("");
  lines.push(
  ...
  lines.push(
    "   Sabaté et al. (2024, bioRxiv, DOI: 10.1101/2024.08.09.605990).",
  );
  lines.push("");
...
async function main() {
  ...
  // Analyze all loci
  const lociNames: (keyof typeof LOCUS_CONFIG)[] = [
    "MYC",
    "IGH",
    "TCRα",
    "SOX2",
  ...
  for (const name of lociNames) {
    ...
    if (result) {
      results.push(result);
      console.log(
        `  ✓ SE Zone Enrichment: ${result.enrichment.seZoneEnrichment.toFixed(2)}x`,
      );
    } else {
  ...
  const meanLoadingFold =
    results.reduce((s, r) => s + r.enrichment.loadingFold, 0) / results.length;
  const meanContactFold =
    results.reduce((s, r) => s + r.enrichment.contactFold, 0) / results.length;
  const meanSEEnrichment =
    results.reduce((s, r) => s + r.enrichment.seZoneEnrichment, 0) /
    results.length;
  const totalDiffCells = results.reduce(
    (s, r) => s + r.enrichment.diffCells,
    0,
  );
  ...
  const report: FinalReport = {
    ...
    hypothesis:
      "H2: Mediator-driven cohesin loading creates spatial contact enrichment",
    loci: results,
    globalStats: {
      meanLoadingFold,
      meanContactFold,
  ...
  for (const r of results) {
    console.log(
      `| ${r.name.padEnd(5)} | ${r.enrichment.seZoneEnrichment.toFixed(2).padStart(16)}x |`,
    );
  }
  console.log("");
