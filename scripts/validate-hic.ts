...
async function runSimulation(params: {
  ...
  console.log(
    `🧬 Running simulation: velocity=${params.velocity}, cohesins=${params.cohesinCount}`,
  );
...
async function validateLoopMode(genomicLoops: Loop[], datasetId: string) {
  ...
  try {
    ...
    console.log(
      `   Region: ${result.chromosome}:${result.region?.start}-${result.region?.end}`,
    );
    ...
    console.log(
      `   Avg loop size (sim): ${(result.avgSizeSimulated / 1000).toFixed(1)} kb`,
    );
    console.log(
      `   Avg loop size (exp): ${(result.avgSizeExperimental / 1000).toFixed(1)} kb`,
    );
...
async function exploreLoops(datasetId: string) {
  ...
  const { downloadLoopList, filterLoops, getLoopAnchors } =
    await import("../src/downloaders/hic");
  ...
  const allChr11 = result.loops.filter(
    (l) => l.chr1 === "chr11" || l.chr1 === "11",
  );
  ...
  if (sizes.length > 0) {
    console.log(
      `   Size range: ${(sizes[0] / 1000).toFixed(0)} kb - ${(sizes[sizes.length - 1] / 1000).toFixed(0)} kb`,
    );
    console.log(
      `   Median size: ${(sizes[Math.floor(sizes.length / 2)] / 1000).toFixed(0)} kb`,
    );
  }
  ...
  for (const loop of allChr11.slice(0, 5)) {
    ...
    console.log(
      `     x1=${loop.x1}, x2=${loop.x2}, y1=${loop.y1}, y2=${loop.y2}`,
    );
    console.log(
      `       → anchors: ${left.toFixed(0)} - ${right.toFixed(0)} (${(size / 1000).toFixed(1)} kb)`,
    );
  }

  // Check HBB region
  const chr11Loops = filterLoops(result.loops, "chr11", {
    start: REGION_START,
    end: REGION_END,
  });
  console.log(
  ...
  if (chr11Loops.length > 0) {
    ...
    for (const loop of chr11Loops.slice(0, 5)) {
      const { left, right, size } = getLoopAnchors(loop);
      console.log(
        `     ${left.toFixed(0)} - ${right.toFixed(0)} (${(size / 1000).toFixed(1)} kb)`,
      );
    }
  }

  // Check wider region around HBB
  const widerRegion = filterLoops(result.loops, "chr11", {
    start: 0,
    end: 10000000,
  });
...
function showDatasets() {
  ...
  for (const d of datasets) {
    console.log(
      `${d.id.padEnd(31)} | ` + `${d.cellLine.padEnd(9)} | ` + `${d.source}`,
    );
  }
...
async function main() {
  ...
  const validationMode = (modeArg ? modeArg.split("=")[1] : "real") as
    | "mock"
    | "real"
    | "strict-real";
  const datasetId = datasetArg
    ? datasetArg.split("=")[1]
    : "rao2014_gm12878_loops";
  ...
  if (!["mock", "real", "strict-real"].includes(validationMode)) {
    throw new Error(
      `Invalid --mode value: ${validationMode}. Use mock|real|strict-real`,
    );
  }

  if (
    validationMode === "strict-real" &&
    !useDownload &&
    !listOnly &&
    !exploreOnly
  ) {
    throw new Error(
      "strict-real mode requires --download to avoid synthetic/offline fallback paths",
    );
  }
  ...
  if (usePowerLaw) {
    ...
    try {
      ...
      if (result.f1Score >= 0.5) {
        ...
        console.log(
          `⚠️  Validation: F1 = ${(result.f1Score * 100).toFixed(1)}% (target: 50%)`,
        );
      }
    ...
    console.log(
      "Using power-law validation (offline). Use --download for real Hi-C data.\n",
    );
    const result = await validatePowerLawMode(simulation.matrix);
    ...
    console.log(
      "  --dataset=ID  Use specific dataset (default: rao2014_gm12878_loops)",
    );
    console.log(
      "  --mode=MODE   Validation mode: mock|real|strict-real (default: real)",
    );
  }
}
