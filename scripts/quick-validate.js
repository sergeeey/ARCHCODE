...
async function validate(params) {
    ...
    const sites = HBB_SITES.map(s => createCTCFSite('chr11', s.pos, s.orient, s.strength));
    ...
    console.log(`   Loops: ${loops.length}, Pearson r: ${validation.pearsonCorrelation.toFixed(3)}`);
...
async function main() {
    ...
    // Best result
    const best = results.reduce((max, r) => r.metrics.pearson > max.metrics.pearson ? r : max);
