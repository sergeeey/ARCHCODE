...
async function readMed1BigWig(bwPath: string): Promise<number[] | null> {
  try {
    ...
    console.warn(
      "[run-fountain-myc] BigWig read failed, using constant signal:",
      (e as Error).message,
    );
    return null;
  }
...
async function main(): Promise<void> {
  ...
  console.log(
    `  beta = ${BETA}, numCohesins = ${NUM_COHESINS}, maxSteps = ${MAX_STEPS}`,
  );
  ...
  for (let step = 0; step < MAX_STEPS; step++) {
    ...
    if (step % 10000 === 0) {
      ...
      console.log(
        `  step ${step}: active cohesins = ${activeCount}, total loops = ${engine.getLoops().length}`,
      );
    }
  }
