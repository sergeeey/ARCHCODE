...
const ctcfDataPath = path.join(
  __dirname,
  "..",
  "data",
  "hbb_ctcf_sites_literature.json",
);
...
for (let i = 0; i < sites.length; i++) {
  ...
  console.log(
    `   ${i + 1}. ${site.position_relative} bp (${site.orientation}) - ${site.source}`,
  );
}
console.log();
...
if (loops.length > 0) {
  ...
  Array.from(uniqueLoops.entries())
    ...
    .forEach(([key, strength], i) => {
      console.log(
        `     ${i + 1}. ${key} bp (strength: ${strength.toFixed(2)})`,
      );
    });
}
console.log();
...
const outputPath = path.join(
  outputDir,
  "archcode_hbb_literature_ctcf_matrix.json",
);
