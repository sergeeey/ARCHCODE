...
const BASELINE_PATH = path.join(__dirname, '..', 'results', 'baseline_s1000.json');
...
function main() {
    ...
    console.log('===================================\n');
    console.log('| Beta | stepLoadProb | Diff Max | Mean|Diff| | NonZero Cells |');
    console.log('|------|--------------|----------|------------|---------------|');

    for (const beta of BETAS) {
        const test = loadJson(path.join(__dirname, '..', 'results', `beta${beta}_s1000.json`));
        if (!test) continue;
        const stats = computeDiffStats(test.heatmap, baseline.heatmap);
        const prob = test.fountainLoader?.stepLoadingProbability ?? 0;
        console.log(`| ${beta.toString().padStart(4)} | ${prob.toFixed(6).padStart(12)} | ${stats.max.toFixed(3).padStart(8)} | ${stats.meanAbs.toFixed(6).padStart(10)} | ${stats.nonZero.toString().padStart(13)} |`);
    }
