...
function runScript(scriptName: string, args: string[]): Promise<void> {
  return new Promise((resolve, reject) => {
    const proc = spawn(
      "npx",
      ["tsx", path.join(__dirname, scriptName), ...args],
      {
        stdio: "inherit",
        shell: true,
      },
...
function computeDiffStats(
  beta5: any,
  beta0: any,
): { diffCells: number; maxEnrichment: number } {
  if (!beta5?.heatmap || !beta0?.heatmap)
    return { diffCells: 0, maxEnrichment: 0 };
...
async function runLocusValidation(
  locus: (typeof LOCI)[0],
  ...
  const beta0StepLoad =
    beta0.fountainLoader?.stepLoadingProbability ??
    beta0.stepLoadingProbability ??
    0;
  const beta5StepLoad =
    beta5.fountainLoader?.stepLoadingProbability ??
    beta5.stepLoadingProbability ??
    0;
...
async function main() {
  ...
  for (const r of results) {
    ...
    console.log(
      `| ${r.locus.padEnd(5)} | ${r.chromosome.padEnd(5)} | ${r.length_mb.toFixed(1).padStart(4)} Mb | ${r.loadingIncrease.toFixed(1).padStart(4)}x | ${r.diffCells.toString().padStart(4)} | ${status} |`,
    );
  }
